...
pro prepare_nssdc_RDF, year, update_only=update_only, listing=listing
...
file_list = file_search('/crater/observatories/wind/ionspec/ionspec'+yyyy+'*.idl', count=count)
if count gt 0 then $
   for i = 0, count-1 do spawn, 'gzip -f ' + file_list[i] ; just for uniformity's sake
file_list = file_search('/crater/observatories/wind/ionspec/ionspec'+yyyy+'*.idl.gz')

; 1.01 Find the files for which there is not yet a CDF, but only if
; /update_only is thrown (otherwise do the whole year)
if keyword_set(update_only) then begin
    idlfiles = file_search('/crater/observatories/wind/ionspec/ionspec'+yyyy+'*.idl.gz', count=nidl)
    cdffiles = file_search('/crater/observatories/wind/ionspec/ionspec'+yyyy+'*.cdf.gz', count=ncdf)
    if nidl gt ncdf then begin
        tk = ncdf + indgen(nidl-ncdf); since the file_list retrieval is chronological, we 
                                ;can just take the ones at the end as
                                ;the yet-to-be-processed
        file_list = idlfiles[tk]
    endif
endif
...
; make a string array to contain output file listing
listing = ''
...
FOR i = 0, n_elements(file_list)-1  do begin 
  ...
  datestamp = strmid(shortname, 11, 8, /reverse)
  slash_chars = strsplit(shortname, '/')
  path = strmid(shortname, 0, slash_chars[n_elements(slash_chars)-1])
  spawn, 'gunzip ' + file_list[i]
  restore, shortname
  spawn, 'gzip ' + shortname
  fsave = path + 'wi_sw-ion-dist_swe-faraday_'+datestamp+'_v01.cdf'
  listing = [listing, fsave]; add this save file to the updated file listing
