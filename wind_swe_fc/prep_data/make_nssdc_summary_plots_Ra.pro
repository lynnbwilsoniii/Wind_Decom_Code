...
pro make_nssdc_summary_plots_Ra, year
...
tk = where(wpera lt 0 or wpera gt 400 or wpara lt 0 or wpara gt 400)
; check that all crazy data are set to fill value
ra = wpera/wpara
flagval = max(ra)
if max(ra[tk]) eq min(ra[tk] ) then print, 'Fills are all good'
...
set_plot, 'ps'
device, /encapsulate, filename = '~/temp/CDF_validation_Ra_' + s_year + '_histograms.eps', $
...
for i = 0, 11 do begin &$
  ...
  if (ntk gt 1) then if (max(na_plot[tk]) ne min(na_plot[tk])) $
    then histoplot, na_plot[tk], charsize = 2, axiscolor = 'black', $
           title = percent + '% fit flag = ' + string(i, format = '(I2)'), xtitle = 'log(wpera/wpara)' $
...
set_plot, 'ps'
device, /encapsulate, filename = '~/temp/CDF_validation_Ra_' + s_year + '.eps', $
  /inches, ysize = 11, xsize = 8.5, /color
...
for i = 0, 7 do begin &$
  ...
  if ntk gt 1 then begin &$
   plot, doy[tk], na[tk], ytitle= 'log(wpera/wpara)', charsize = 2, psym = 3, $
