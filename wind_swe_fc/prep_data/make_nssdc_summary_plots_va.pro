...
pro make_nssdc_summary_plots_va, year
...
flagval = max(va)
if max(va[tk]) eq min(va[tk] ) then print, 'Fills are all good'
...
set_plot, 'ps'
device, /encapsulate, filename = '~/temp/CDF_validation_va_' + s_year + '_histograms.eps', $
...
for i = 0, 11 do begin &$
  ...
  if (ntk gt 1) then if (max(va_plot[tk]) ne min(va_plot[tk])) $
    then histoplot, va_plot[tk], charsize = 2, axiscolor = 'black', $
...
set_plot, 'ps'
device, /encapsulate, filename = '~/temp/CDF_validation_va_' + s_year + '.eps', $
  /inches, ysize = 11, xsize = 8.5, /color
...
for i = 0, 7 do begin &$
  ...
  if ntk gt 1 then begin &$
   plot, doy[tk], va[tk], ytitle= 'n_a', charsize = 2, psym = 3, $
   xrange = [min(doy[tk]), max(doy[tk])], xstyle =1, /yl  &$
   for j = 0, 11 do begin &$
     subtk = where(fit[tk] eq j, nsubtk) &$
     if nsubtk gt 0 then oplot, doy[tk[subtk]], va[tk[subtk]], color = j*24, psym = 3, thick = 3 &$
    endfor &$
  endif &$
endfor
