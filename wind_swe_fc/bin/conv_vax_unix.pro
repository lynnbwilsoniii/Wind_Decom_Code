; @(#)conv_vax_unix.pro  VERSION 1.2    7/28/94   16:13:24
...
;  Check to see if VARIABLE is defined.
 ...
 if N_params() LT 1 then begin
      print,'Syntax - var_unix = conv_vax_unix( var_vax ,[ TARGET_ARCH = ])
      return,0
 endif
...
CASE var_type OF
  ...
  6: return, complex( conv_vax_unix( float( variable ), TARGET_ARCH=target ),  $
		    conv_vax_unix( imaginary( variable ), TARGET_ARCH=target ) )
  ...
  8: BEGIN				; structure
	...
	for t=0,Ntag-1 do  var_out.(t) = $
				conv_vax_unix( variable.(t), TARGET_ARCH=target )
	return, var_out
       END
