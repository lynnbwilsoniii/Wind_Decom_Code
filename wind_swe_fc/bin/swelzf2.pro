; @(#)swelzf.pro  VERSION 1.2    7/28/94   16:13:38
...
; The calling procedure is SWELZF.PRO.
	...
	endcase
		...
		endcase
		  ...
		  print,'Done'
             ...
             2: spawn,'avg_cal2 caldata ; avg_cal2 -l caldata > caldata.avg'
             3: spawn,'avg_mod moddata > moddata.avg ; more moddata.avg'
             4: calgraph
             5: begin
		  point_lun,-lundat,pos		;check pos in file
		  mjf = bytarr(11560)
		  readu,lundat,mjf
		  point_lun,lundat,pos		;reset pos in file
		  print,mjf(0:67),format='((68z3.2/))'
		  conv1=conv_vax_unix(mjf(0))
		  conv2=conv_vax_unix(mjf(4))
		  conv3=conv_vax_unix(mjf(8))
		  conv4=conv_vax_unix(mjf(12))
		  print,conv1,conv2,conv3,conv4
		end
		;print,'Pssst...  Look in the black box on the third shelf...'
           endcase
	endcase
        else:
...
;=================== CALLING PROCEDURE: swelzf ================================

PRO swelzf2, GROUP = GROUP
...
; readf,lun,lzpath & free_lun,lun
  ...
  if infile eq '' then infile='lz/default.dat'
...
;widget interface to select and read lz records
  ...
  cbase1split=widget_base(cbase1,/row)
  cbase1left=widget_base(cbase1split,/column)
  cbase1right=widget_base(cbase1split,/column)
  ...
  [ 'file header',$ 
    ...
    'genl hk data',$
    'fc data'  ]
  wa.button1(2)=cw_bgroup(cbase1left,list,row=7,label_top='Print Options')
  ...
  [ 'compile all fc data',$ 
    ...
    'plot caldata',$
    'print major frame'  ]
  wa.button1(3)=cw_bgroup(cbase1right,list,row=7,label_top='Other Options')

  rbase1=widget_base(cbase1,/row)
...
XMANAGER, "swelzf", wa.base, GROUP_LEADER = GROUP  ;hand off to manager
