...
/* %title "Description of Perl 5 interface to libsms" */
/* %module libsms */
/* %style before, skip=1 */
/* libsms-h2i-end */
...
/* the bits that are set in each of the above flags */
enum EDBQualityBits {
...
#define WORD unsigned short int  /* for TM integers; use this type so that it
				  could in principle be redefined to a fraction
				  of a short if a short were to get longer 
				 (as per Dave Gell's suggestion) */
...
typedef struct edbheader_dc {
  ...
  */
  ...
  /* byte 3 */
  int fMPresent; /* T/F, is Mass sensor prsent? */
...
typedef struct edb_raw_high { /* raw, high bit rate mode */
  /* EDB header */
  BYTE h[11];

  /* SWICS(X) core area -- 116 */
  BYTE xbr0[16];
  BYTE xbr1[16];
  BYTE xbr2a[16];
  BYTE xbr2b[16];
  ...
  /* SWICS PHA Words, sized for maximum, see p291 in DPU manual */
  BYTE xpha[672]; /* 168 words * 4 bytes/word */

  /* STICS core area -- 180 bytes */
  BYTE thmr[96];
  BYTE tsmr[24];
  BYTE tbr0[8];
  BYTE tbr1[8];
  ...
  /* STICS PHA Words, sized for maximum, see p291 in DPU manual */
  BYTE tpha[608]; /* 152 words * 4 bytes/word */

  /* MASS core area -- 27 bytes */
  ...
  /* MASS PHA Words, sized for maximum, see p291 in DPU manual */
  BYTE mpha[762]; /* 381 words * 2 bytes/word */

} EDB_R_H;

typedef struct edb_raw_low { /* raw, low bit rate mode */
  /* EDB header */
  BYTE h[11];

  /* SWICS core area -- 62 bytes */
  BYTE xbr0[8];
  BYTE xbr1[8];
  BYTE xbr2x[8];
  BYTE xbr2y;
  ...
  /* SWICS PHA Words, sized for maximum, see p291 in DPU manual */
  BYTE xpha[324]; /* 81 words * 4 bytes/word */

  /* STICS core area -- 90 bytes (180 bytes commutted over 2 edb) */
  BYTE thmr[96];
  BYTE tsmr[24];
  BYTE tbr0[8];
  BYTE tbr1[8];
  ...
  /* STICS PHA Words, sized for maximum, see p291 in DPU manual */
  BYTE tpha[296]; /* 74 words *  4 bytes/word */
  ...
  /* MASS PHA Words, sized for maximum, see p291 in DPU manual */
  BYTE mpha[362]; /* 181 words * 2 bytes/word */

} EDB_R_L;

#define EDB EDB_R_L  /* temporarily for convenience */

typedef struct xpha {/* decoded SWICS PHA word */
...
typedef struct tpha {/* decoded STICS PHA word */
  /* in order from low bit to high bit */
  int stopId; /* stop ID */
...
/*********************************/
/* housekeeping data block (HDB) */
/*********************************/
typedef struct hdb { 
  /* an HDB is 280 bytes in science mode; 231 bytes in maneuver mode */
...
/* Decommuted HDB -- contains misc. decommuted values from HDB */
typedef struct decomm_hdb {

  /* STICS stepping tables -- element x is for step x */
  ...
  /* MASS stepping tables -- element x is for step x */
...
/* %subsection "SMS Return Codes", pre */
...
*/
/* %section "Typical Perl Usage", pre, after */
/* 
# open file
$file = "/home/winddata/LV1/2001/wi_lz_sms_20010225_v01.dat";
$result = libsms::smsOpenFile($file);

# set trace to 1 for more verbose output from libsms
libsms::smsstrace(0);
...
/* prototypes for general utility functions */
unsigned long lBuildWord(unsigned char *,long int,long int);
double dBuildWord(unsigned char [],long int,long int);

/* prototypes for libsms perl-accessible functions */
/* libsms-h2i-begin */ 
/* %section "Global Functions" */
/* %subsection "Main Cycle Reading Functions" */
/* Opens data file, returns one of the SMS return codes. */
int smsOpenFile(char [MAXFILENAME]);

/* Closes open data file, returning SMSSUCCESS (see above) if no problems
...
/* subsection "Misc. Utility Functions" */
...
/* %subsection "Flags, Configuration and Quality-Related Functions" */
...
/* Set libsms debug level to i.  These debug values are primarily for
   ...
   level 3 -- adds output as each H/EDB is found in record (lots of output)
...
/* smsgedbqual -- get EDB quality flag for EDB[nedb].  This is a bit field;
   ...
   2       STICS core area (all rates, DVS, mode flag) is bad.
   3-5     not used
   6       Entire EDB is suspect; This means a problem was found which will
             have ill-defined effects on the EDB (e.g. npha out of range)
   7       Entire EDB is bad.
...
/* %section "Specific instrument functions" */
/*********/
/* SWICS */
/*********/

/* %subsection "SWICS" */
/* %subsubsection "engineering rates" */
...
/* %subsubsection "decommuted HDB items" */
...
/* %subsubsection "non-engineering rates" */
...
/* %subsubsection "STICS PHA", before */
