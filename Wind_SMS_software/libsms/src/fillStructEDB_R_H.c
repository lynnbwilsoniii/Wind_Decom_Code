/** \file fillStructEDB_R_H.c
    \brief Decode EDB array into detailed structure (for high bit rate mode)
*/
/**
...
/*
   ...
   $Id: fillStructEDB_R_H.c,v 1.2 2005/11/29 19:22:11 jfeeman Exp $
...
EDB_R_H fillStructEDB_R_H(BYTE abEDB[],EDB_H_DC *ph_dc, short quality){
  int i,j;
  int iinit; /* (temporary) initial value for loop counter */
  int imax; /* (temporary) maximum for loop counter */
  
  char thisprog[] = "fillStructEDB_R_H"; /* name of this program */
  ...
  /* xcore */
  /* In high bit rate mode, the SWICS core data area contains 116
     bytes, 62 bytes in low bit rate mode */
  imax = 116;
