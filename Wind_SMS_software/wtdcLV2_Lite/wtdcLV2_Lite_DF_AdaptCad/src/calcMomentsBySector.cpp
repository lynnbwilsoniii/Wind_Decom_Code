...
vector<momentsBySector> calcMomentsBySector(double df[][16][3], double df_err[][16][3], double vel_part[], double cyc_accum, double velCutOff) {
	...
	for (int j=0; j< 16; j++) {
		...
		// units: km/s
		if (momentsSector.f0 > 0.0) {
			v = momentsSector.f1/momentsSector.f0;
		} else {
		...
		for (int i=0; i<32; i++) {
			for (int k=0; k<3; k++) {
				momentsSector.f2 += solid_angle[k]*df[i][j][k]*pow((vel_part[i]-v),2.0)*pow(vel_part[i],2)*deltaV[i];
			}
		}
		...
		for (int i=0; i<32; i++) {
			for (int k=0; k<3; k++) {
				momentsSector.f2_err += pow(solid_angle[k]*df_err[i][j][k]*pow((vel_part[i]-v),2.0)*pow(vel_part[i],2)*deltaV[i],2.0);
			}
		}
