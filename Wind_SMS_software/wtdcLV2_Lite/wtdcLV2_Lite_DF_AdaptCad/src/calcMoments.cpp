...
Inputs: df - the ions distribution function
		df_err - the measurement error from the distribution function
...
moments calcMoments(double df[][16][3], double df_err[][16][3], double vel_part[], double cyc_accum, double velCutOff) {
	...
	for (int i=0; i<32; i++) {
		for (int j=0; j<16; j++) {
			for (int k=0; k<3; k++) {
				f2 += solid_angle[k]*df[i][j][k]*pow((vel_part[i]-v),2.0)*pow(vel_part[i],2)*deltaV[i];
			}
		}
	}
	...
	for (int i=0; i<32; i++) {
		for (int j=0; j<16; j++) {
			for (int k=0; k<3; k++) {
				f2_err += pow(solid_angle[k]*df_err[i][j][k]*pow((vel_part[i]-v),2.0)*pow(vel_part[i],2)*deltaV[i],2.0);
			}
		}
	}
	f2_err = sqrt(f2_err);
	...
	if (f0 > 0.0 && f2 > 0.0) {
		v_th_err = v_th*sqrt(pow((f2_err/f2),2)+pow((f0_err/f0),2));
	} else {
		//fill with a nan if both the first and second moment are junk
		v_th_err = -sqrt(1);
	}	
