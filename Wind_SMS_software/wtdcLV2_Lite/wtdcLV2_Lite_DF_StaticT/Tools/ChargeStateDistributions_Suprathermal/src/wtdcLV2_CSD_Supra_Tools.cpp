...
vector<string> getFileNames(string atom, string timePeriod, string dataDir, int atomicNumber) {
	...
	string fileBase = "wtdcLV2_Lite_SCFrame_";
	
	//based on the element we generate a list of files
	vector<string> fileNames;
	
	if (atom == "H") {
		fileNames.push_back(dataDir+fileBase+atom+"+_DF_"+timePeriod+".dat");
	} else if (atom != "H") {
		for (int i=0; i<atomicNumber; i++) {
			if (i == 0) {
				fileNames.push_back(dataDir+fileBase+atom+"+_DF_"+timePeriod+".dat");
			} else {
				fileNames.push_back(dataDir+fileBase+atom+intToString(i+1)+"+_DF_"+timePeriod+".dat");
			}
		}
	} 
	//return the filelist
