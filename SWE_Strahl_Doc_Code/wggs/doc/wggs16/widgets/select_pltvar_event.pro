...
case event.id of 
  ...
  WDGT.newstrl_pad: begin ;          "Pitch-angle Dist. Descriptors" list.
     empty_val = 'empty' ;                        Define empty-list-value.
     print,'"Pitch-angle Dist. Descriptors" list.' ;         Diag. output.
     print,'event.value ',event.value ;          Name of selected entry.

     ;            UPDATE current state of the selected-descriptors list...
     if (n_elements(pad_list_curr) eq 0l) then begin ; Curr. list EMPTY...
        if (event.select eq 1l) then pad_list_curr = event.value
     endif else if (pad_list_curr[0] eq empty_val) then begin ;   EMPTY...
        if (event.select eq 1l) then pad_list_curr = event.value   
     endif else begin ;     Curr. list NON-empty--add or remove entries...
        if (event.select eq 1l) then $ ;        Add curr. entry to list...
           pad_list_curr = [pad_list_curr,event.value] $
        else begin ;                       Remove curr. entry from list...
           w_nev = where((pad_list_curr ne event.value),w_nev_count)
           if (w_nev_count ne 0l) then $ ;   Other entries loc. in list...
              pad_list_curr = pad_list_curr[w_nev] $ ; Keep other entries.
           else pad_list_curr = empty_val ;    Keep only empty-list-value.
        endelse
     endelse

     ;           DISPLAY current state of the selected-descriptors list...
     if (n_elements(pad_list_curr) ne 0l) then begin ;   Non-empty list...
        help,pad_list_curr & print,pad_list_curr,Format='(TR5,A)'
     endif else begin ;                   Empty list (shouldn't happen)...
        help,pad_list_curr & beep & print,'Warning! Descriptor-list lost!'
     endelse 
  endcase
  ; end of: NEW!! MPH ---------------------------------------------------
