...
for idatyp=0,n_elements(d.flnm)-1 do begin         
  if d.flnm(idatyp) ne '' then begin
     ...
     2 : begin
         ;!!!!!!!!!!!!!
         ;The following if-then block should be commented or deleted. It's wrong.
         ;if d.datype(idatyp) eq 'swe_fpitch' then begin
           ;numen=n_elements(d.swe_pdat(d.ndx(0,idatyp)).vsteps)
           ;d.pnlist.list(d.pnlist.offs(idatyp)+1:$
           ;   d.pnlist.offs(idatyp)+1+d.pnlist.len(idatyp)-1)=''
           ;d.pnlist.list(d.pnlist.offs(idatyp)+1:d.pnlist.offs(idatyp)+1+numen-1)=$
           ;string(volt_en(d.swe_pdat(d.ndx(0,idatyp)).vsteps,/en),format='(f8.1)')
         ;endif
