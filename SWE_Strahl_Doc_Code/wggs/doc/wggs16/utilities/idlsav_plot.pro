...
print,'Hit return to continue' & answ='' & read,answ & if answ ne '' then stop
  ...
  nsaved=n_elements(idlsav)    ;number of saved variables 
  indx_to_plot=indgen(nsaved)  ;saved variable indices to be plotted
  ;indx_to_plot=[3,5,8]
  
  npl=n_elements(indx_to_plot)      ;number of moments variables to plot
  pos=fltarr(4,npl)
  pos,npl,pos
  ...
  for j=0,npl-1 do begin
    ...
    if j eq npl-1 then xtickname=idlsav(npl-1).xtickname(0:xticks)
