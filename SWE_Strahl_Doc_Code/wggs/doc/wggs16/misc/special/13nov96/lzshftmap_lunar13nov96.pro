...
cowen_13nov96,msecd,phi
help,msecd,phi         ;phi=phi (radians) required to correct calulated phi's

for i=0,n_elements(msecd)-1 do print,i,msecd(i),phi(i),phi(i)/!dtor
msec_ecl0=5528037l        ;begin eclipse
msec_ecl1=11236016l       ;end eclipse
...
;recn_range=[1,fh.nmf] ;read entire file testing for eclipse times
recn_range=[110,fh.nmf]
...
;start spin loop
  for ispin=0,nspins-1 do begin
    ...
    print,'recn, ispin.... ',recn, ispin, msec_ecl0,msec_ecl1,timpb5(2)
...
lunar.dphi=interpol(phi,float(msecd),float(lunar.timpb5(2)))
...
for isp=0,nsp-1 do begin
  ...
  phi_true=dblarr(ndets,nvsteps,nsectors)    ;degrees
...
;save,filename=getenv('IDLSAV')+'newlunar_13nov96.idlsav',lunar
