...
pro proc_rec_special,tmmode_ihk=tmmode_ihk,lpr=lpr,elec_ion_m1=elec_ion_m1,$
...
;cuurent_recn is in science mode 
  ifsci=1
...
else if scimode_ihk_0 eq 0 or scimode_ihk_0 eq 1 then $
    hvtbl16(*,0)=lz.mf(hkm1(24).offs+indgen(16))
...
;get spin period
  ...
  if lpr then print,$
    'proc_rec: time tag:  tjd      secdy  hh:mm:ss.ms  spincnt  spinperiod'
...
;get mode dependent portion of hv table this mjf
if scimode_ihk_1 eq 2 or scimode_ihk_1 eq 11 then $
    hvtbl64(*,1)=lz.mf(hkind(ghk(27).offs+indgen(ghk(27).ln))) $
  else if scimode_ihk_1 eq 0 or scimode_ihk_1 eq 1 then $
    hvtbl16(*,1)=lz.mf(hkm1(24).offs+indgen(16))
...
point2:     
  ;if (scimode_ihk_0 eq 0 or  scimode_ihk_0 eq 1) then goto,point3

;check the stored mode1 and mode2 hv table 
  if (scimode_ihk_0 eq 2 or  scimode_ihk_0 eq 11) then $ 
    eleion_sweep=sweepmode(veis_hvtbl,mode=2) else $
  if (scimode_ihk_0 eq 0 or  scimode_ihk_0 eq 1) then $
...
point3:    ;reset recn to input_recn and read record
...
;do mode dependent unpacking
  ...
  if (scimode_ihk eq 0 or  scimode_ihk eq 1) and elec_ion_m1 eq 0 then begin
    ;test for mode change
      if swest.mode_this_file eq 2 then begin
        stop,'change from mode2 to mode1; mode1 background and glint required'
      endif 
    mode1_special,lpr=lpr,noglnt=noglnt,err=err 
  endif else if scimode_ihk eq 2 or scimode_ihk eq 11 then begin
    ;test for mode change
      if swest.mode_this_file eq 1 then begin
        stop,'change from mode1 to mode2; mode2 background and glint required'
      endif 
    mode2,eleion_sweep,lpr=lpr,noglnt=noglnt,err=err
  endif else if (scimode_ihk eq 0 or  scimode_ihk eq 1) and $
