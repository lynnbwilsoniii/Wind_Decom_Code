...
;filen=getenv('WGGSBASE')+'swe/cal/gains/rgcoef_98jun02_mod'
restore,filen
...
;pb5_end=long([1998,         135,    0])
;elapsec_end=elapsec_change4_5

elapsec_end=elapsec_change4_4
intrvl_after = elapsec ge elapsec_end
...
relgain=fltarr(ndets)

if intrvl ge 0 and intrvl le intrvl_end then begin

  for i=0,ndets-1 do relgain(i)=$
    coeff(0,i,intrvl)+coeff(1,i,intrvl)*elapsec+coeff(2,i,intrvl)*elapsec^2
  
  if intrvl eq 7 then begin
    ;relgain=[1.00, 1.69,  2.02,  3.79,  1.34,  0.92]   ;for date=19970928
    ;a=1.5  ;special1
    ;relgain=[1.00, 1.69,  a*2.02,  a*3.79,  1.34,  0.92]  ;special1

    ;b=2.0    
    ;relgain=[b*1.00, b*1.69,  2.02,  3.79,  b*1.34,  b*0.92]    ;special2
    
    ;b=2.0    
    ;relgain=[b*1.00, b*1.69,  b*2.02,  b*3.79,  b*1.34,  b*0.92]    ;special3
  endif
