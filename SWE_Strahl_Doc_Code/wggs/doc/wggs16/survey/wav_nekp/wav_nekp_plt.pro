...
hrday=(d.wav_nekpdat(d.ndx(0,idatype):d.ndx(1,idatype)).ta-d.refsec)/3600.d
  
;wav_nekpvar returns data (varbl_orig) for selctd variable (pm(i).varname)
   varbl_orig=wav_nekpvar(pm(i).varname)

;filter elec density according to quality factor
  id1=d.ndx(0,idatype)
  id2=d.ndx(1,idatype)
  qndx=id1+indgen(id2-id1+1)
  qmin=150. ;200.
  wok=where(d.wav_nekpdat(qndx).q ge qmin)
  varbl_orig=varbl_orig(wok)
  hrday=hrday(wok)
