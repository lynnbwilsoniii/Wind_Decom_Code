...
; MPH (10/10/2003)--This updates the tool's panel-list to reflect correct mode.
numen = n_elements(pdat[d.ndx[0,idatype]].vsteps) ;   Number of energies avail.
if ((ymd_pb5(long(wst.indate)))[0] ge 2002l) then begin ;     2002-and-after...
   ...
   print,"Using NEW MODE (Strahl) panel list." ;                    Diagnostic.
   numen = numen-1 ; The number of new-mode energies is one less than old-mode.
   d.pnlist.list  [ (d.pnlist.offs[idatype]+1):$ ; Use new mode (mode7) list...
                   ((d.pnlist.offs[idatype]+1)+(numen-1))] = $
                 string(volt_en_strl(pdat[d.ndx[0,idatype]].vsteps[1:*],/En),$
                        Format='(F8.1)') ; Drop bottom (empty) new-mode v-step.
   ; After correcting list of available energies, remove all information about
   ;             final "extra" energy-level which does not exist in new mode...
   d.pnlist.list  [ (d.pnlist.offs[idatype]+1)+ numen    ] = '' ;    Remove en.
   d.pnlist.dtp   [ (d.pnlist.offs[idatype]+1)+ numen    ] = '' ;    Remove en.
   d.pnlist.ev_val[ (d.pnlist.offs[idatype]+1)+ numen    ] = 0 ;     Remove en.
   d.pnlist.len   [                idatype  ] = numen+1 ; (+spect.)  Remove en.
endif else begin ; Otherwise, 2001-and-before, so not in new instrument mode...
   ; Note: Any electron pitch-angle data from 2001-and-before must be old-mode.
   print,"Using OLD MODE (VEIS) panel list." ;                      Diagnostic.
   d.pnlist.list[ (d.pnlist.offs[idatype]+1):$ ;    Use previous mode's list...
                 ((d.pnlist.offs[idatype]+1)+(numen-1))] = $
