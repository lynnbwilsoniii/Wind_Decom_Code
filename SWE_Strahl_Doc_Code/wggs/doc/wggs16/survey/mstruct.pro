...
;find range for time axis 
  ...
  for ipnl=0,n_elements(pnlsel)-1 do begin  
    ...
    if d.pnl[pnlsel[ipnl]].dtp eq 'swe_moments' or $ ; !!!! MPH mod., 08/08/03.
       d.pnl[pnlsel[ipnl]].dtp eq 'swe_newmomf' then swe_moments=1
  
    if swe_moments eq 1 or $ ;  (covers "new" and "old" moments) MPH, 08/08/03.
       d.pnl[pnlsel[ipnl]].dtp eq 'swe_fpitch' or $
       d.pnl[pnlsel[ipnl]].dtp eq 'swe_newstrl' or $ ; (covers only "new") MPH.
