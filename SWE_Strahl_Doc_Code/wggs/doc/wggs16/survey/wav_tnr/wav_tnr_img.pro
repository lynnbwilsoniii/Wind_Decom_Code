pro wav_tnr_img,z,x,dateymd,pos=posn,$
...
flow=d.wav_tnr_spctrm.fhzmin  ;4.
fhigh=d.wav_tnr_spctrm.fhzmax  ;245.146
fspace=.188144e-1
funit='kHz (1mTNR)'
linlog=1
...
;if necessary, rebin time dimension to the pixel resolution
   ...
   if szarr(1) gt ntlim then begin  
     sclfctr=fix(szarr(1)/ntlim)+1
     ntmx=sclfctr*(szarr(1)/sclfctr)
     case wst.rebin of
     0: arrayb=congrid(arrayb,ntmx/sclfctr,szarr(2))
     ;0: arrayb=rebin(arrayb(0:ntmx-1,*),ntmx/sclfctr,szarr(2))
     else:
     endcase
   endif
   help,arrayb
