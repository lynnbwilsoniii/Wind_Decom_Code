...
pro proc_rec,tmmode_ihk=tmmode_ihk,lpr=lpr,elec_ion_m1=elec_ion_m1,$
  ...
  norelgains=norelgains,in_mode7=in_mode7
...
; Note: New mode 7 changes denoted by '!' before and after lines that are new.
...
if recn le fh.nmf then read_rec,date_time else return ;stop,'end of input data file'
...
;determine which science mode
  if lz.mf(hkm7(4).offs) eq 7 then scimode_ihk_0=7 else $
  scimode_ihk_0=get_bits(lz.mf(ihk(2).offs),ihk(2).bv(0).p,ihk(2).bv(0).n)

in_mode7=scimode_ihk_0
print,'!!!!!!!!!!!!!!!Science Mode:                        ',scimode_ihk_0
print,'lz.mf(hkm7(4).offs):                                ',lz.mf(hkm7(4).offs)
print,'get_bits(lz.mf(ihk(2).offs),ihk(2).bv(0).p,ihk(2).bv(0).n): ',$
...
if recn lt fh.nmf then recn=recn+1 else begin
  ;use last determined spin period
  if sp.lst_spinp ne 0 then sp.spinp=sp.lst_spinp else sp.spinp=3.0
  goto,point3  
endelse
read_rec,date_time
...
if lz.mf(hkm7(4).offs) eq 7 then scimode_ihk_1=7 else $ 
...
if scimode_ihk_1 ne 4 and scimode_ihk_1 ne 7 then begin
...
if scimode_ihk_0 ne 6 and scimode_ihk_1 ne 6 and $
   scimode_ihk_0 ne 7 and scimode_ihk_1 ne 7 then begin
...
;instr hk tm and sci mode  
  tmmode_ihk=get_bits(lz.mf(ihk(2).offs),ihk(2).bv(1).p,ihk(2).bv(1).n)
  if lz.mf(hkm7(4).offs) eq 7 then scimode_ihk=7 else $
...
;do mode dependent unpacking
  ...
  endif else if scimode_ihk eq 7 then begin
