pro fmulti_plot,x_im_sz,y_im_sz,$
  pltype=fpl_sel_ind,$
...
common sharefmutli,fmulti

if swest.plotf then begin
  set_plot,'x'
  swest.iplotf=swest.iplotf+1 ;< (swest.nplotf-1)
  ;stop
  if swest.iplotf eq 0 then begin
    swest.nplotf=6
    window,0
    !p.multi=[0,3,2,0,0]
    ;!p.multi=[0,sqrt(swest.nplotf),sqrt(swest.nplotf),0,0]
    fmulti=replicate({x:fltarr(129),y:fltarr(129),yrange:fltarr(2),$
          xrange:fltarr(2),xticks:0,subtitle:'',xminor:0},swest.nplotf)
  endif
  wset,0
endif  
...
for istack=0,nstack-1 do begin
  ...
  0: begin
       contour,z,x,y,levels=cl,max_value=maxzval,xrange=xrange,yrange=yrange,$
         ...
         device=dev,title=title,xtitle=xtitle,$
  ...
  1: begin
       ...
       plot_io,x(wF),F(wF),xrange=xrange,xstyle=1,xticks=xticks,$
         ...
         ;position=position,ytitle='reduced log!d10!n F',

       if swest.plotf then begin       
         fmulti(swest.iplotf).x=x
         fmulti(swest.iplotf).y=F
         fmulti(swest.iplotf).yrange=Frange
         fmulti(swest.iplotf).xrange=xrange
         fmulti(swest.iplotf).xticks=xticks
         fmulti(swest.iplotf).subtitle=subtitle
         fmulti(swest.iplotf).xminor=xminor
       endif  
...
if swest.iplotf eq swest.nplotf-1 then begin
  !p.multi=0
  swest.iplotf=-1
  svflnm=getenv('IDLSAV')+strcompress(fmulti(0).subtitle,/remove_all)+'_fmulti'
  save,filename=svflnm,fmulti
endif  
