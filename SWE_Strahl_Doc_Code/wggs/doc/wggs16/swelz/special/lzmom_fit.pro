...
openr,lundate,getenv('IDLSAV')+'lzmom_dates_test',/get_lun
...
;if date eq '19970110' then recs=[211,211]
if date eq '19970110' then recs=[1174,1174]
if date eq '19950625' then recs=[579,579]  
...
;--------------- start spin loop
  ...
  ;both glint and the floor, are set to 1 count
    ...
    hypersurf0=iflgs(8)
    nobnfit0=iflgs(9)
    ;print,'0: ',recn,ispin,iflgs(8),iflgs(9),iflgs(4)
    ;print,'0: ',dnout,teout,sqrt(total(uout*uout))*1e-5
    
    ;----------------- end doing moments vpot=0 ------------------------------


;<<<<<<<< do model fit with vpot=0 >>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>

f=fblk(*,*,0:nsectors-1)
f=reform(transpose(f,[0,2,1]),n_vdets*nsectors,n_vesteps_trunc)
f_av=fltarr(n_vesteps_trunc)
for i=0,n_vesteps_trunc-1 do begin
  wpos=where(f(*,i) gt 0,nwpos)
  if nwpos gt 0 then f_av(i)=total(f(wpos,i))/nwpos
endfor

en_break=100. 
vsteps_halo=where(energy gt en_break,nstepshalo)
velhalo=velocity(vsteps_halo)
enhalo=energy(vsteps_halo)
fhalo=fblk(*,vsteps_halo,0:nsectors-1)
chalo=float(icnts_mb(*,vsteps_halo,0:nsectors-1))
fhalo=reform(transpose(fhalo,[0,2,1]),n_vdets*nsectors,nstepshalo)
chalo=reform(transpose(chalo,[0,2,1]),n_vdets*nsectors,nstepshalo)
fhalo_av=fltarr(nstepshalo)
chalo_av=fltarr(nstepshalo)
for i=0,nstepshalo-1 do begin
  wpos=where(fhalo(*,i) gt 0,nwpos)
  if nwpos gt 0 then begin
    fhalo_av(i)=total(fhalo(wpos,i))/nwpos
    chalo_av(i)=total(chalo(wpos,i))/nwpos
  endif
endfor

fhalo_av=fhalo_av(where(fhalo_av ne 0))
chalo_av=chalo_av(where(fhalo_av ne 0))
vhalo=velhalo(where(fhalo_av ne 0))
ehalo=enhalo(where(fhalo_av ne 0))

;do gauss fit
;yfit=gaussfit(vhalo,fhalo_av,a,nterms=5)

;do kappa fit
;19950625: halodens=coredens*0.025  vhalo=2*vcore   kappa_halo=6.5
;19970110, recn 211: halodens=coredens*0.02  vhalo=1.75*vcore  kappa_halo=3.0
;19970110, recn 1174: halodens=coredens*0.02  vhalo=2.0*vcore  kappa_halo=2.0

vmax=max(velocity)/1e8
coredens=dnout   ;10.
vcore=sqrt(eavg*1.6022e-12/9.1095e-28)/1e8      ;2.
halodens=coredens*0.02 ;0.02
vhalo=2.0*vcore  ;sqrt(5.)*vcore
uhalo=0.
kappa_halo=2.0 ;3.;5.  ;7.  ;3.  ;4.  
...
window,0
plot,velocity,f_av,/ylog,psym=1,title=date
oplot,velx(where(velx ge 0))*1e8,10.^logfx(where(velx ge 0))
oplot,velcore(where(velcore ge 0))*1e8,10.^logfcore(where(velcore ge 0)),color=75
oplot,velhalo(where(velhalo ge 0))*1e8,10.^logfhalo(where(velhalo ge 0)),color=125

;oplot,vhalo,fhalo_av,color=75
;oplot,vhalo,yfit,color=125
...
if do_scpot ne 0 then begin
    ...
    hypersurf1=iflgs(8)
    nobnfit1=iflgs(9) 
    ;print,'1: ',iflgs(8),iflgs(9),iflgs(4)
    ;print,'1: ',dnout,teout,sqrt(total(uout*uout))*1e-5
...
skip2:
    ...
    ;make output data structure assignments
      ...
      mdata.misc2(21)=hypersurf0 ;=1 if hypersurf check in patch fails, 0 if ok
      mdata.misc2(22)=hypersurf1
      mdata.misc2(23)=nobnfit0 ;=1 if bn computation in patch fails, 0 if ok
      mdata.misc2(24)=nobnfit1
      ;if hypersurf eq 1 or nobnfit eq 1 then ipatch in iflgs(4) is reset to 0
      ;and moments are computed with no patch
