...
;plots veis counts data (all steps) vs  sunangle
...
for jdet=0,ndets-1 do begin
  x=acos(sin(vsmjf.theveis(jdet)*!dtor)*$
              cos(vsmjf.phiveis(jdet,*,*)*!dtor))/!dtor
  y=abs( vsmjf.veis(jdet,*,*,ispin) )
  vmn=-1 & vmx=-1
  ...
  oplot,x,y,psym=4,symsize=0.25,color=clr(hardcopy)
  wglnt=where(vsmjf.xveis(jdet,*,*,ispin) eq -1)
  if wglnt(0) ne -1 then begin
   if hardcopy eq 0 then oplot,x(wglnt),y(wglnt),psym=4,symsize=0.50,$
     color=wst.clr_green $
