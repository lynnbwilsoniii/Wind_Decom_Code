...
;computes pitch angle averages over pi bin averages at given energy
;pa bins = 0-60 deg, 60-90 deg, 90-120 deg, 120-180 deg are each pi solid angle
...
openr,lundate,getenv('IDLSAV')+'lzpitchavg_dates',/get_lun
...
;start spin loop
  for ispin=0,nspins-1 do begin
    ...
    if do_swf ne 0 then begin
      ...
      interpol_ion,$
      ionkpdat.ta,ionkpdat.v,iondx,vsmjf.suntim_vsbl(ispin),refsec,vsw 
    endif else vsw=0
    ...
    0: begin      ;do electrons       
     ...
     ;measured data will be put in pitch angle bins pbin degrees
       pbin=[0.,60.,90.,120.,180.]
       np=4 
       ;dp=180/np
       ;ip=fix(pa/dp)*dp
     ...
     ;do binning 
       ...
       for i=0,nvsteps-1 do begin
         sum=0. & nsum=0
         w180=where(pa(i,*) eq 180.0)
         if w180(0) ne -1 then pa(i,w180)=179.9
         for jp=0,np-1 do begin
         ;wh=where(ip(i,*) eq jp*dp and cp(i,*) ge 0,nwh)
         wh=where(pa(i,*) ge pbin(jp) and pa(i,*) lt pbin(jp+1) $
              and cp(i,*) ge 0,nwh)
...
;                 version 05 intended for distribution
    ...
    mfl=getenv('MPNEW')+wst.lzdate+'_'+mfltr+'.pitavg'   ;'.pit'
