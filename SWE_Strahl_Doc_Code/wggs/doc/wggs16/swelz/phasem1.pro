...
;determine spin phase at each bci (each data sample) for mode 1 (also mode 4, 6)
...
if keyword_set(phisun) eq 0 then phisun=0.d
...
phiveis_sp=dblarr(n_vdets,n_vesteps,n_sectors)
...
;spin phase angle of veis data samples relative to direction of sun;
    ...
    for isector=0,n_sectors-1 do begin
      ...
      for ivestep=0,n_vesteps-1 do begin
         ibci=ibci+1
         phiveis_sp(0:2,ivestep,isector,*)=phiv1(ibci)-phivdet(0:2)
         phiveis_sp(3:5,ivestep,isector,*)=phiv2(ibci)+phivdet(3:5)
      endfor
    endfor

;make range of phi 0:360
  w=where(phiveis_sp ge 360.,nw)
  if nw gt 0 then phiveis_sp(w)=phiveis_sp(w)-360.

  w=where(phiveis_sp lt 0.,nw)
  if nw gt 0 then phiveis_sp(w)=phiveis_sp(w)+360.
