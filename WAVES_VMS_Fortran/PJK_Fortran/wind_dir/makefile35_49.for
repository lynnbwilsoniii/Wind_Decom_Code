...
!	program		apmplot
!
c	This is makefile35, derived from makefile11, which was
c	derived from apmplot--but treats a long run of antenna resistance
c		measurements
c	I intend to use the data Stuart sent, which has flux from 20000718
c		to 2000/12/05 and np from 2000/07/18 to 2000/08/27
c	The purpose of this mod is to try to find the photoemission 
c	characteristic of the X antenna, i.e. Cu-Be
...
c		print*,'check year,=',year
	...
	nday = scet
	type*,'position channel pointer to',scet,nday
c
...
C	type*,'channel no',ch
	...
	type*,'return size,scet',ret_size,scet
c
	call w_ur8_to_ymd(scet,yyyy,mon,dd,hh,mm,ss,ms)
	print*,'check time',scet,yyyy,mon,dd,mm
c
...
c	item = 'CAL_GENERATOR'
	...
	if(cal_on.eq.1) then
	  ...
	  go to 1001		! 1 means radio cal on.
	endif
	item = 'DPU_MAJOR_FRAME'
	ok = w_item_i4(ch, item, major, 1, ret_size)
...
c		USE TIME
c
c	go back for another event if drive (and relay?) are off
c
	   item = 'APC_X'		! word 69
	   ok = w_item_i4(ch, item, Ix_on, 1, ret_size)
	   if (.not. ok) write(6,2) 'cannot get item ',item, ', ok=', ok
C	print*,'x_on ',Ix_on
	if(Ix_on.ne.0) go to 1001
c
...
c	   calculate day of year and fraction of day
...
c
c	x resistors are:0	10 Meg	34.8 total
c			1	350 Meg 375  total
c			2	10 Meg  34.8 total
c			3	1000 Meg  1025 total
c			4	100 Meg	125 total
c			5-7	subtract 4		
c
...
C
	print 555,s_scet,scet,is,av(1,is),drive(is)
  555	format(i10,i8,f10.4,i5,2f8.3)
 2001	GO TO 1000
C 	end do
 2000	continue
...
c
	if(nstart.gt.0) then
	  open(unit=89,file='ztable.dat',status='old',access='append')
c
	  dens = 0.
	  dsigma = 0.
	  cdfn = 0.
...
C	    solve for base and antenna resistances
	  ...
	  ENDIF
c
	 write(89,989),yyyy,mon,dd,itstart,itend,resis,frms,angle,
     1	   dens,dsigma,etemp,tsigma,tempi,sigmait,eflux,baseR,XR,YR
...
c		print*,'x index',itemp
		rm = xresistor(itemp+1)
c	      	print*,'x resistor',itemp,rm
		rm = 34.8
...
c	open(unit=90,file='accrres.dat',type='old',access='append')
	...
	etemp = 0.
	eflux = 0.
c	write(90,5001) file(31:38),itstart,itend,rxsun,rxshd,
c     1		rysun,ryshd,dens,etemp,	eflux
 5001	format(A9,2i5,7f8.2)
c	close(unit=90)
	return
	end
