...
C	THE + AND - X SIDES ARE DIVIDED INTO 4 PARTS
C	THIS DIFFERS FROM MODEL 6 IN THAT Z ANTENNAS ARE ADDED
C
...
C	NO, IT'S SAME DISTANCE BETWEEN ANTENNA BASES
C
	RFLAT = RWIND			!TEMPORARY
	RBOOM = .12			!WIDTH OF BOOM
	ZMONOPOLE = 5.28
	FREQ = 9.075 E6
C
...
C
C	DO THE SIDES
C		FIRST, THE MAG BOOM SIDE (+X PLATE) IN 4 PIECES
C
	NCRNS = 4
	IREC = 1
	X1 = RFLAT
	...
	WRITE(99,*) X1,-Y1,Z1
	WRITE(99,*) X1,Y1,Z1
	WRITE(99,*) X1,Y1,RBOOM
	WRITE(99,*) X1,-Y1,RBOOM
	WRITE(99,1001) NCRNS,SEGM,IREC,IPN,IGENSIDE,ZSHT(1)
	WRITE(99,*) X1,-Y1,RBOOM
	WRITE(99,*) X1,S0,RBOOM
	WRITE(99,*) X1,S0,-RBOOM
	WRITE(99,*) X1,-Y1,-RBOOM
	WRITE(99,1001) NCRNS,SEGM,IREC,IPN,IGENSIDE,ZSHT(1)
	WRITE(99,*) X1,S0,-RBOOM
...
C
C
C		 THE -X MAG BOOM SIDE IN 4 PIECES
C
	NCRNS = 4
	IREC = 1
	X1 = -RFLAT
...
C
	...
	Z(I) = HEIGHT/2. 
	WRITE(99,*) X(I),Y(I),Z(I)
	DHZ = .5*ZMONOPOLE				! 2 SEGMENTS IN 5.8 M.
	Z(I) = Z(I) + 2.*DHZ
	DO I = 31,32
	  X(I) = X(I-1) 
	  Y(I) = Y(I-1) 
...
C				
	DO I = 1,22				! X ANTENNA
	  ...
	  WRITE(99,*) IA(I),IB(I)
	ENDDO
	DO I = 23,28				! Y ANTENNA
	  IA(I) = I+1
	  IB(I) = IA(I) + 1
	  WRITE(99,*) IA(I),IB(I)
	ENDDO
	DO I = 29,34				! Z ANTENNA
	  IA(I) = I+1
	  IB(I) = IA(I) + 1
	  WRITE(99,*) IA(I),IB(I)
	ENDDO
C
C	READ 14, FEED POINTS, FOUR LOADS AND TWO GENERATORS
C
C	  WRITE(99,1002) NAS(I),IAB(I),NPLA(I),VGA(I),ZLDA(I),BDSK(I)
	...
	ZCAP = 87.5E-12
	XC = 1./TWOPI/FREQ/ZCAP
	ISEGM = 28				! Z antenna
	IAB(5) = 1
	VGA(1) = CMPLX(0.,0.)
	ZLDA(1) = CMPLX(0.,-XC)
	WRITE(99,1004) ISEGM,IAB(5),VGA(1),ZLDA(1)
	ISEGM = 29
	IAB(6) = 0
	WRITE(99,1004) ISEGM,IAB(6),-VGA(1),ZLDA(1)
C
