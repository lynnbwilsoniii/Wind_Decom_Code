...
C		COUNT ONLY POS TO NEG
	...
	ENDDO
	print*,'zero crossings found, IZCNT=',izcnt
...
C
	...
	IF(IREMOVE.NE.0) THEN
	  ...
	  ENDDO
	  IZCNT = IZCNT-NREMOVE
	  PRINT*,'ZEROS REMOVED, IZCNT CUT TO ',IZCNT
	ENDIF
C
	IF(IREMOVE.EQ.0) THEN
	  ...
	  ENDDO
	  IZCNT = IZCNT-NREMOVE
	  PRINT*,'ONLY ZERO INTERVALS REMOVED, IZCNT CUT TO ',IZCNT
	ENDIF
        AVRFREQ=0.
        IF(IZCNT.GT.1) THEN
...
C
C	WRITE ELECTRIC FIELD ENERGY TO E2ENERGY.DAT
C		WHAT IS CALCULATED IS .5*EPSO*E**2 * DT
C		SO NEED TO MULTIPLY BY PACKETWIDTH**2 AND DX/DT = VSW
C
          item = 'RX_PERIOD_R4'
          ok = w_item_R4(ch, item, DTIME, 1, return_size)
          EFFLEN = 41.1                           ! X ANTENNA   9-NOV-96
          IF(IRX.EQ.2.OR.IRX.EQ.5) EFFLEN = 3.79  ! Y ANTENNA   "
          IF(IRX.EQ.3.OR.IRX.EQ.6) EFFLEN = 2.17  ! Z ANTENNA   "
        print*,'effective length,DTIME',efflen,DTIME
C       CHANGE TO V/meter, SO ACORR IN SUBROUTINES IS 1.
	ESQ = 0.
	DO NE = 1,2048
	  ESQ = ESQ + .5*EPS0*(DATA(NE)/EFFLEN)**2*DTIME
	ENDDO
	OPEN(UNIT=21,FILE='E2ENERGY.DAT',STATUS= 'OLD',ACCESS='APPEND')
	WRITE(21,1021) S_SCET,ITEMP,TDS_CHANNEL,PARX(IRX),AVRFREQ,ESQ
 1021	FORMAT(I10,I8,I10,I3,A6,E12.3,E12.3)
	CLOSE(UNIT=21)
C
	number_event = itemp
C	CALL PLTDSFR(-2)
...
C	pubcombo makes 3 panels, data, freq from zero x, spectumn
C	CALL PUBCOMBO(-3)
C	CALL PUBCOMBO(7)
C	pubcombo2 makes 2 panels, data and spectrum
C	CALL PUBCOMBO2(-1)		! needs work
C	CALL PUBCOMBO2(3)
	CALL PUBCOMBO2(-3)		! save
c	IF(IPROCESS.LE.1) CALL DETAIL(-2,900,1100)
C	CALL DETAIL(-2,924,1124)
C	CALL DETAIL(-2,1,200)
c	CALL DETAIL(-2,1847,2047)
C	CALL DETAIL(-4,900,1100)
	NPLOTS=NPLOTS+1
...
C
C	PUT ON CYLOTRON FREQUENCY
C
	CALL MGORELOCATE(PP(900),.2*YMIN+.8*YMAX)
	BMAG = SQRT(BX**2 + BY**2 + BZ**2)
	WRITE(STR,1031) .028*BMAG
 1031	FORMAT('f\Dc\De =',f6.2,' kHz')
	CALL MGOLABEL(19,STR)
C
	YTITLE = GY2
	TRANGE = GY2-GY1
	TINC = .1*TRANGE
...
C	  IF(N.EQ.12) YTITLE = YTITLE + 2.*TINC
C	  IF(N.EQ.19) YTITLE = YTITLE - TINC
	  CALL MGOGRELOCATE(XTITLE,YTITLE)
	  IF(N.NE.13.AND.N.NE.14) CALL MGOLABEL(12,TITLE(N))
	ENDDO
C
...
c	  CALL MGOLABEL(13,'S/C SUN CLOCK')
c	  YTITLE = YTITLE-.6*TINC
c	  CALL MGOGRELOCATE(XTITLE,YTITLE)
...
C
	  ...
	  CALL MGOSETEXPAND(1.)
	CALL MGOSETEXPAND(.8)
	CALL MGOPLOTID(EVENT,'[.WIND]TDSPRO,PUBCOMBO')
	CALL MGOSETEXPAND(1.)
C
C	PLOT TDS DATA IN PHYSICAL UNITS
C
...
C	CALL MGOPOINTS(60.,1,PP,YY,NP)
	...
	IF(TDS_CHANNEL.LE.2) CALL MGOXLABEL(9,'FREQ, kHZ')
C
C	PUT ON CYCLOTRON FREQUENCY
C
	BMAG = SQRT(BX**2+BY**2+BZ**2)
	WRITE(STR,1031) .028*BMAG
1031	FORMAT('f\dc\de =',f6.2,' kHz')
	CALL MGORELOCATE(PP(700),.15*YMIN + .85*YMAX)
	CALL MGOSETEXPAND(.8)
	CALL MGOLABEL(19,STR)
C
	YTITLE = GY2
...
C
	  ...
	  CALL MGOSETEXPAND(1.)
	CALL MGOSETEXPAND(.8)
	CALL MGOPLOTID(EVENT,'[.WIND]TDSPRO,PUBCOMBO2')
	CALL MGOSETEXPAND(1.)
C
C	PLOT TDS DATA IN PHYSICAL UNITS
C
...
C
	...
	YMAX = YMAX + .05*YRANGE
	YMIN = YMIN - .05*YRANGE
	print*,'spectrum ymin,max',ymin,ymax
C	YMIN = AMAX1(YMIN,YMAX-70.)
c*******
	CALL MGOSETLIM(-.017*XMAX,YMIN,XMAX,YMAX)
