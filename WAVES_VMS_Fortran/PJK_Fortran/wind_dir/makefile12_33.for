...
c
C	A PROGRAM TO SEARCH FOR LANGMUIR WAVES IN THE SOLAR WIND IN TDSF. 
C		OR FILL
c
C	TEMPORARILY, FINDS BIG TYPE III LANGMUIR WAVES
C
...
C	  ok2 = W_ITEM_I4(ch, item, N2DATA, 2048, ret_size)
	...
	ENDIF
C*****
	IF(MAXCH1.LT.107.AND.MAXCH2.LT.90) GO TO 100
C*****
...
C
	IF(RGSE.LT.6.4D05) THEN			! LIMIT 100 RE
...
C	    OK = W_ITEM_R8(ch, item, SCET8, 1, ret_size)
	    ...
	    PRINT*,RGSE/6.38E3,'INSIDE 100 RE',SCETI4
...
C
	IF(FREQKHZ.GT..8*FP) THEN
	  PRINT*,'EVENT NO.',NO_EVT,' FREQ', FREQKHZ,' MAX',
     1		MAXCH1,MAXCH2
	  IFOUND = 0
c		to exclude cals
	  if(sceti4(2).lt.010000.or.sceti4(2).gt.011000) then
	      write(54,1054) SCETI4,NO_EVT,MAXCH1,MAXCH2,
     1		XMAX/.0411,YMAX/.00379,FREQKHZ,FP
 1054	  FORMAT(I10,I7,I10,2I4,2E11.3,2F8.2)
	  endif
	  GO TO 100
	ENDIF
...
C
C	  WRITE(90,1011) sceti4,NO_EVT,FREQKHZ,PX,PY,SLOPE,ANGTOB,
C     1	     AZMAG,MAGEL,FP,FCE,BETA,XCOUNT,YCOUNT
C 1011	format(I10,I7,I10,F7.2,2E10.3,F8.3,2F7.1,I5,F7.3,F7.4,F8.4,2F7.0)
C
