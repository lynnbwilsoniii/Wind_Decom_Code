...
c	implicit	none
	...
	character*32	item
	integer*4	ios,yyyy,idoy,msec,dputime
...
C	   item = 'DPU_MF'
C	   ok = w_item_i4(ch, item, dputime, 1, return_size)
C
	   item = 'EVENT_SCET_R8'
...
c     1     ,scet(2)
	   ...
	   do i=1,511
		...
		TPOWER = TPOWER + RAW(I+1)**2
	   end do
	   WRITE(88,*) SCET,FFT_CHANNEL,DPUTIME,TPOWER
...
C
C	THIS PART OF THE PROGRAM IS A COMPLICATED EFFORT TO KEEP A DENSITY
C	OF STRIPES ON THE SHADE PLOT WHICH IS UNIFORMLY DISTRIBUTED IN TIME,
C	EVEN THOUGH THE EVENTS ARE NOT.
C
c	NPT = 1
C
C	DO N = 1,NPTMX-1
...
cold	  CALL MGOGRELOCATE(GX1,TOP-50.)                    
 ...
 802	  FORMAT(2I6)
	  ...
	  CALL MGOGRELOCATE(GX1,TOP-170.)
	  CALL MGOPUTLABEL(32,SCET,3)
