...
C
	    ...
	    DOWHILE(FDAY.LE.FDAYL)
	      ...
	      swetime = FDAY*tfact
c		get electron density from swe and temperature from 3dp
	      CALL INTERP(T3DP,DENS,NSWE,SWETIME,TE0,ERR) 
C
C	WEL IS IN (V/M)**2
	      WEL = .01809*TE0*DENS(N)	
  	      PP(N) = fday*tfact
	      NPPNT = N
	      YY(N) = WEL
 ...
 201	    YMIN = AMAX1(YMIN,0.)
	    ...
	    CALL MGOYLABEL(13,'nT\de (V/m)\u2')
...
c		B at SWE time
	      CALL INTERP(TMFI,BX,NMFI,PP(N),BX0,ERR)
	      CALL INTERP(TMFI,BY,NMFI,PP(N),BY0,ERR)
	      CALL INTERP(TMFI,BZ,NMFI,PP(N),BZ0,ERR)
	      CALL INTERP(TMFI,BMAG,NMFI,PP(N),BMAG0,ERR)
	      YY(N) = (BX0*VX(N)+BY0*VY(N)+BZ0*VZ(N))/BMAG0
	      IF(YY(N).LT.5.E7)  YMAX = AMAX1(YY(N),YMAX)
	      IF(YY(N).GT.-50000.) YMIN = AMIN1(YY(N),YMIN)
	    ENDDO
 202	    XRANGE = PP(NPPNT) - PP(1)
	    XLST = PP(NPPNT)
	    CALL MGOYLABEL(12,'V dot B/Bmag')
	    CALL MGOXLABEL(16,'HOURS OF THE DAY')
	  ENDIF
C
	  IF(JW.EQ.3) THEN
C
C	PLOT DYNAMIC SW PRESSURE, RATIO TO AVERAGE
c	    CALL MGOTICKSIZE(0.,0.,-1.,-1.)
	    N = 0
	    DOWHILE(FDAY.LE.FDAYL)
	       N = N+1
...
C	PRINT*,'IN PLOT, FILE=',FILE
	CALL MGOSETEXPAND(1.)
	  CALL MGOSETEXPAND(.8)
	  CALL MGOPLOTID(FILE(31:42),'[.WIND]SWSTUFF')
	  CALL MGOSETEXPAND(1.)
C
	ENDDO
	IF(ITERM.LT.0) THEN
