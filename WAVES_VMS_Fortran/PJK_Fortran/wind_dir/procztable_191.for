...
C	WITH ZTABLE.MGO OR ZTABLE3.MGO
C
...
C	MAKE TABLE FOR PLOTTING
 ...
 1037	FORMAT(I10,4F9.2,F6.3,F9.2,F6.3)
	...
	ENDIF
	IF(STZ.LE.1.7.AND.FSTD.LT..2) THEN
	  ISZ = ISZ+1
	  RZZ(ISZ) = RZ
	  FFZ(ISZ) = FLUX
C	  DD(ISZ) = DENS
...
C
	do ii = 1,is
	  print*,'ls',ii,bb(ii),finv(ii)
	enddo
	CALL LSFIT(BB,FINV,WT,IS,BBF,RBASE0,FRMS)
	PRINT*,'BBF,RBAS0',BBF,RBASE0
C
C	MAKE CURVES FOR PLOTTING
C
	DO N = 1,200
...
C
	VMTE = CLIGHT*SQRT(8.*TE/PI/EVME)   ! TE AND EVME IN ELECTRON VOLTS
...
c	print*,'antcurr,earea,parea',antcurr,earea,parea
c	print*,'antpot,antres',antpot,antres
c	antrest = tphi/flux/qesi
c	print*,'formula resistance',antrest
c	if(1) stop
C
...
C	print*,'antpot,pcur,diff',antpot,pcur,diff
	  IF(DIFF*DIFFSV.LE.0.) THEN		!ROOT FOUND, REFINE
...
C	print*,'antpot,pcur,didv,antres',antpot,pcur,didv,antres
	    ...
	    IF(DIDV.NE.0) ANTRES = -1./DIDV
	print*,'antpot,pcur,didv,antres',antpot,pcur,didv,antres
	    ANTPOT = ANTPOTT
	    RETURN
	  ENDIF
	  DIFFSV = DIFF
