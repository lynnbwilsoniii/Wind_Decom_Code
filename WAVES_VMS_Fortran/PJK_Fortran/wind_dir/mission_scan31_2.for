...
C 1012	format(2X,'TIME OF CHANGE',4X,'DRV  RX  RY  RZ  VX  VY  VZ  DRV V' )
	...
	OPEN(UNIT=73,FILE='[KELLOGG.WIND]ORBIT.TABLE',STATUS='OLD',READONLY)
	OPEN(UNIT=56,FILE='MAKEFILE31.RESULTS',STATUS='OLD',ACCESS='APPEND')
 100	continue
...
C	OPEN(UNIT=89,FILE='DATA_A:[KELLOGG]TDSF.DAT',STATUS='NEW')
C	DO ISPS = 1,4
C	    WRITE(89,*) NBCK(1,ISPS),NBCK(2,ISPS)
C	    DO N = 1,1025
C	      WRITE(89,*) N,(TDSFBCK(N,1,ISPS),TDSFBCK(N,2,ISPS))
C	    ENDDO
C	ENDDO
	WRITE(89,*) SCETSTR,SCETEND
	write(89,1088) FHIST,NHISTST
	print*,'fhist',fhist
 1088	FORMAT(14I5) 
	    do irx = 1,2
	      do ifr = 1,14
		freq = FHIST(ifr)*1875./2048.
c		freq = ifr*120000./2048.
		write(89,*) 'rx',irx,'  freq',freq
C		  range is -10 to 90 dB
	          write(89,1089) (hist(ifr,irx,nhist),nhist=1,100) 
 1089		  format(10F8.0)
              enddo
	    enddo
C
	print*, 'STOP IN MISSION_SCAN18'
	CLOSE(UNIT=89)
	CLOSE(UNIT=56)
	STOP
	END
