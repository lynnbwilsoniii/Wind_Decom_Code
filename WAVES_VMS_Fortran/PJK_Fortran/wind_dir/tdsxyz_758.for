...
C**********INTERPOLATE FOR B
	     ...
	     ok = w_item_R8(ch, item, BTIMS, 3, return_size)
    	     PRINT*,'BXS',BXS(1),BXS(2),BXS(3)
	     PRINT*,'TIM',BTIMS(1),BTIMS(2),BTIMS(3)
		BX = BXS(1) + 
     1	(BXS(2)-BXS(1))*(SCETT-BTIMS(1))/(BTIMS(2)-BTIMS(1))
		BY = BYS(1) + 
     1	(BYS(2)-BYS(1))*(SCETT-BTIMS(1))/(BTIMS(2)-BTIMS(1))
		BZ = BZS(1) + 
     1	(BZS(2)-BZS(1))*(SCETT-BTIMS(1))/(BTIMS(2)-BTIMS(1))
    	     PRINT*,'B',BX,BY,BZ
C	     	BX = 150.
C		BY = 250.
		BZ = 300.
