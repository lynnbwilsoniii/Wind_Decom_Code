...
C
      ...
      DATA NPR /0/
      DATA CAP /220.E-12/
      DATA TILT /3./                             !SUN-SPACECRAFT AXIS ANGLE
...
CT      DO 100 NHALF = 1,2
      DO 100 NHALF = 1,10
	PARES = NHALF*2.E7
      AANTT = TWOPI*ANTR*ANTL + 2.*HATA               !AREA FOR EL. PICKUP
      ESAT = .25*DENSW*VTEL*AANTT*1.602E-19
      CALL INIT(F,DF)
      PRMT(3) = 1.1
      PRMT(4) = 1.E-5
C      DO 10 N = 1,16
...
CT      DO 20 N = 1,180
	...
	VMIN = AMIN1(VMIN,F(1))
	RES = .01/(CUR-CURD)
	PRINT 101,F(2),F(1),CUR,CURC,RES
	NPL = NPL+1
	PDATA(NPL,1) = F(2)-360.
	PDATA(NPL,2) = F(1)
   20  CONTINUE
