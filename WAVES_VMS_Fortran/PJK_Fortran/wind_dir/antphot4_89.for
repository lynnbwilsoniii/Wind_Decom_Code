      PROGRAM ANTPHOT4
C
C	THIS CORRESPONDS TO ANTPHOT3 AT MEUDON.  IN MINN, ANTPHOT3 DOES
C	THE DIFFERENCE BETWEEN TWO ANTENNAS WHICH ARE NOT QUITE COLINEAR
C	 AND ANTPHOT4 INCLUDES THE  EFFECT OF RC COUPLING IN THE PREAMP.
C
C	THIS PROGRAM IS INTENDED TO CALCULATE THE POTENTIAL OF A
C	MONOPOLE ANTENNA
C	IN THE SOLAR WIND, AS A FUNCTION OF ROTATION ANGLE.  THE ANTENNA
C	IS ASSUMED TO HAVE HATS OF AREA HATA, ON ITS ENDS.  
C
	COMMON /PARAMS/ RSC,HSC,HANT,ANTR,ANTL,HATA,PI,PARES
	...
	DIMENSION PDATA(500,3),RESLST(11)
	DATA RESLST /2.E11,5.E9,2.E9,1.E9,5.E8,2.E8,1.5E8,1.E8,
     1    .7E8,.5E8,.2E8/
	DATA PI /3.14159265/                
C   HELIOS
	DATA ANTR,ANTL /.317,1600./             !ANTENNA DIM. IN CM., HELIOS
C      DATA TILT /.25/                             !SUN-SPACECRAFT AXIS ANGLE
        DATA SPINP /1./                            !SPIN PERIOD IN SEC.
	DATA PHCURD /40.E-9/              !STD PHOTO EM. CURRENT, AMPS/CM**2
	DATA DENSW /45./                         !SOLAR WIND DENS IN /CM**3
	DATA RSC /86./                          !SPACECRAFT RADIUS IN CM
...
C
C
	DATA TPHE1 /3./                          !PHOTO EL TEMP IN EV 
C	DATA TPHE1 /1.04/                     !PHOTO EL TEMP IN EV, LAI AVR.
...
CT	DATA PARES /2.E11/                    !CURRENT SUPPLY RESISTANCE = oo
	DATA PARES /2.E11/                       !CURRENT SUPPLY RESISTANCE
	DATA ANTR,ANTL /.019,4500./          !ANTENNA DIM. IN CM., WIND X
C	DATA ANTR,ANTL /.019,750./           !ANTENNA DIM. IN CM., WIND Y
C	DATA ANTR,ANTL /.635,750./              !ANTENNA DIM. IN CM., .5 INCH
C	DATA ANTR,ANTL /1.43,600./           !ANTENNA DIM. IN CM., 1. INCH, Z
C
C	DATA HATA /171./                         !HAT AREA IN CM**2, LONG WIND
	DATA HATA /1./                         !HAT AREA IN CM**2
C
      DATA NPL /0/
...
C
	PRINT*,'WITH SHADOW'
C
	WRITE(29,*) DENSW,TESW,TISW,VELSW,ANTL
C	DO 789 NRS = 1,11
C	PARES = RESLST(NRS)
...
C
 104  FORMAT(' CAP'E12.3'  TILT'F6.2' DEG., ANT.FLOAT.POTENTIAL',F8.2)
C
...
C      ENDDO
C      PHISV = F(1)
C      CALL RKGS(PRMT,F,DF,2,IHLF,FDERIV,OUTPUT,AUX)
...
C      DO 20 N = 1,25
      ...
      DO K = 1,2
        DF(K) = WT(K)
      ENDDO
      CALL RKGS(PRMT,F,DF,2,IHLF,FDERIV,OUTPUT,AUX)
...
C	PRINT 101,F(2),F(1),CUR,CURC,RES
 ...
 100  CONTINUE
C
      PRINT*,'MAX AND MIN ANT POT. WRT PLASMA',VMAX,VMIN
      IF(PARES.LT.1.E11) THEN
	RAMAX = RMAX*PARES/(PARES-RMAX)
	RAMIN = RMIN*PARES/(PARES-RMIN)
      ELSE
	RAMAX = RMAX
	RAMIN = RMIN
      ENDIF
      PRINT*,'MAX AND MIN ANT. RESISTANCE',RAMAX,RAMIN
	WRITE(29,1234) PARES,SUPPLY,VMAX,VMIN,VMAX-VMIN,RAMAX,RAMIN
 1234	FORMAT(E14.4,4F9.2,2E13.4)
 789	CONTINUE 
...
C      PRINT*,'CURR COMPS',CURC
   ...
   1  CONTINUE
   2  PHIST = PHIS 
      IF(CURS-CUR.NE.0.) PHIST = PHIS - (PHIS-PHI)*CURS/(CURS-CUR)
...
C
	...
	CH(9) = 'YLABEL ANT. POT. WRT PLASMA'
	CH(10) = 'ID'
	CH(11) = 'END'
      IF(IHARD.EQ.1) THEN
	CH(11) = 'HARDCOPY'
	CH(12) = 'END'
      ENDIF
	CALL MONGO(11+IHARD,CH,500,3,PDATA)
      RETURN
      END
