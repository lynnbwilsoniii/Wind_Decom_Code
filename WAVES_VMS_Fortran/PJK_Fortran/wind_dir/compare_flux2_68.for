	PROGRAM COMPARE_FLUX2
C
C	COMPARE ACCURATE FLUXES FROM STUART AND CDF DATA
C		THIS VERSION USES STUARTS DENSITY DATA TO CALCULATE
C		TRUE TEMPERATURE AND FLUX
C
...
c
	OPEN(UNIT=72,FILE='COMPAREFLUX.RESULTS',STATUS='OLD',
     1		ACCESS='APPEND')
C
 10	write(6,6)
...
C
c	do is = 1,50
c	  write(66,*) time3dp(is),dens3dp(is),etemp(is)
c	enddo
C	  if(is.gt.20) stop
C	  go to 100
C
...
C
	  READ(83,*,END=142,ERR=150) NYR,MO,NDAY,NHR,MIN,NSEC,EFLUXT
	  NSS = NSS+1
...
C
C	DETERMINE TIME INTERVALS AND PROCESS THIS TABLE
C
	NS = 1
	STARTTIME = .5D00*(TIME3DP(NS) + TIME3DP(NS+1))
 140	CONTINUE
	DO NS = 2,NSS
	  TEFLUX = 0.
	  SEFLUX = 0.
	  ECOUNT = 1.E-9
	  ENDTIME = .5D00*(TIME3DP(NS) + TIME3DP(NS+1))

C
C	  AVERAGE DENSITY DATA
C
	  CALL DENSAV(STARTTIME,ENDTIME,DENSITY,STD,NERR)
C	  print*,'d chk',starttime,endtime,density,NERR
	  IF(NERR.NE.0) GO TO 162
C	  SEARCH CDF DATA (NCDF = 1,100) FOR CLOSEST TIME
C
	  TDIFF = 1.D00
	  DO NCDF = 1,100
C	  print*,'time3dp,flux',time3dp(ncdf),ns,fluxtime(ns)
	    IF(TIME3DP(NCDF).GT.FLUXTIME(NSS)) GO TO 160
 ...
 160	  CONTINUE
C
C	  NOW NBEST IS THE CDF DATA NEAREST STUART'S FLUX

c	  PRINT*,'ns,fluxtime,ncdf,time3dp ',ns,FLUXTIME(ns)
c     1		,ncdf,time3dp(ncdf)
C	  XTEMP = EFLUX(NS)/DENS3DP(NBEST)
	  XTEMP = EFLUX(NS)/DENSITY
	  EFFTEMP = TWOPI*EVME*(XTEMP/CLIGHT)**2	! IN EV
	  RATIO = 1.
	  IF(ETEMP(NBEST).GT.0.) RATIO = EFFTEMP/ETEMP(NBEST)
	  WRITE(72,1072) YYYYMMDD,FLUXTIME(NS),DENSITY,EFLUX(NS),
     1	    DENS3DP(NBEST),ETEMP(NBEST),EFFTEMP,RATIO
 1072	  FORMAT(I9,F12.5,F8.3,E12.4,F8.4,F8.4,F10.4,F8.4)
  162	  CONTINUE
	  STARTTIME = ENDTIME
	ENDDO
C
...
C
 200	CONTINUE
	...
	STOP
	END
	SUBROUTINE DENSAV(STARTTIME,ENDTIME,DENSITY,SEDENS,NERR)
C
	REAL*8 STARTTIME,ENDTIME
C
C	PRINT*,'IN DENSAV',STARTTIME,ENDTIME
	SEDENS = 0.
	TEDENS = 0.
	DCOUNT = 1.E-9
C
 150	CONTINUE
	  READ(84,*,END=160,ERR=150) NYR,MO,NDAY,NHR,MIN,NSEC,DENST
c	print*,'read 84',starttime,endtime
	  FDAYF = NHR/24. + MIN/1440. + NSEC/86400.
	  NDAY = STARTTIME
c	print*,'sub',nday,starttime
     	  DENSTIME = DFLOAT(NDAY) + DBLE(FDAYF)
	  IF(DENSTIME.LT.STARTTIME) GO TO 150
	  IF(DENSTIME.GT.ENDTIME) GO TO 160
	  SEDENS = SEDENS + DENST**2
	  TEDENS = TEDENS + DENST
	  DCOUNT = DCOUNT + 1.
	GO TO 150
 160	CONTINUE
	NERR=0
	IF(DCOUNT.LT..5) NERR=1
	DENSITY = TEDENS/DCOUNT
	SEDENS = SEDENS/DCOUNT - DENSITY**2
	SEDENS = SQRT(AMAX1(SEDENS,0.))
	RETURN
	END
