...
c
C	MAKEFILE 32, A PROGRAM TO SEARCH FOR ION ACOUSTIC WAVES WITH 
C	MAGNETIC FIELD, DERIVED FROM MAKEFILE6, SEARCH FOR LION ROARS IN TDSS. 
...
C
	...
	INTEGER*4 ISOURCE(6),NO_EVT(4),ERT(2),YYYY,DD,hh,mm
	CHARACTER*120 JUNK
...
C
	IF(IHEADER.EQ.0) THEN
	  ...
	  WRITE(90,1090) 
	  WRITE(90,1091) 
 1090	  FORMAT(5X,'DATE,TIME'4X'ERT'4X'EVT NO.','  GLT  '4X'PWR'6X'B2xy',
     1	  5X,'R21'3X'R31'3X'COSKB'3X'f# ','FREQSP ','  BW  ','FREQRV ',
     2    ' REV',3X,'fp',2X,'fce',4X,'beta')
	  WRITE(37,1091) 			! BLANK LINE
	  WRITE(37,2037)
 2037	  FORMAT('  EVT NO.',1X,'EVENT',3x,' DATE,TIME',4X,'P E ',4X,
     1     'CRITERIA',5X, 'f#',5X,'f Hz',7X,'BW',9X,'PWR',5X,'REVS',1X,
...
C
	  ...
	  WRITE(91,2090) 
	  WRITE(91,1091) 
 2090	  FORMAT('   EVT NO.',3X'f# ','FREQSP ','  BW  ','FREQRV ',
     2    ' REV',3X,'fp',2X,'fce',4X,'beta')
...
C
	IF(TDS_CHANNEL.LE.2) GO TO 100
C
	ITEM = 'SOURCE'
	ok = W_ITEM_I4(ch, item, IRX, 1, ret_size)
	ISOURCE(TDS_CHANNEL) = IRX
	LD = 4
	IF(ISOURCE(TDS_CHANNEL).GT.6) LD = ISOURCE(TDS_CHANNEL) - 6
	RCVR(LD) = IRX
	CHSV(LD) = TDS_CHANNEL
C
	IPROS = IPROCESS
...
C
	...
	ELSE
	  ...
	  ENDDO
	  FREQMIN = 0.
	  FREQMIN = 3.6				! test comparison with xyz
C	  FREQMIN = 4.5				! 
	  FREQMAX = 938.
	  ITEM = 'EVENT_NUMBER'
...
C
	  IF(LD.NE.3) THEN
	    ...
	    LD = 3
	  ENDIF
ct	print*,'calling tds_phys,ipros,LD = ',ipros,LD
...
C
	I3E3B = 0
	DO N = 3,6
C	  print*,'3B check',n,ISOURCE(n),no_evt(n-2)
	  IF(ISOURCE(N).GT.6) I3E3B = I3E3B + 1 
	ENDDO
	IF(I3E3B.LT.3) THEN
    	  PRINT*,'NOT 3 B s, I3E3B.LT.3, =',I3E3B,' get another sequence'
	  GO TO 100
	ENDIF
C
...
C
	...
	if(edist.lt.100.) THEN
	  PRINT*,'WIND IS WITHIN 100 RE'
	  go to 100
	ENDIF
C
...
c********** end histogram
C
C	THE TEST FOR SIGNIFICANT SIGNAL POWER.  REQUIRE THAT 4 
...
c	IF(NO_EVT(1).EQ.6158946) THEN
	IF(SCETI4(2).EQ.143605) THEN
C	IF(NO_EVT(1).EQ.6039022) THEN
	  ITERM = 3
	  ITERM = -2
...
C	IF(1) GO TO 100
C
	PX = (EVAL(1)+EVAL(2))/2048.          ! MEAN SQUARE OF B (TRANSVERS)
	R21 = EVAL(2)/EVAL(1)
	R31 = EVAL(3)/EVAL(1)
	if(px.gt.10.**-2.5) write(77,*) no_evt(1),px
C
...
C
	  WRITE(90,1011) sceti4,ERTDAY,EVENT(1:1),NO_EVT(1),IGL,
     1	   OBSPWR(5),PX,R21,R31,COSKB,NFMAX,FREQSP,BW,FREQR,REVS,
     1	   FP,FCE,BETA
...
C		tail end for narrow printer at Vaughn
C
	  WRITE(91,2011) NO_EVT(1),NDCS,FREQSP,BW,FREQR,REVS,
     1	   FP,FCE,BETA
 2011   format(I10,I5,F6.1,f6.1,F6.1,F7.1,F6.1,F6.3,F7.3)
C
