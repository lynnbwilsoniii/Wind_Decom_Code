...
C
	IF(ITERM.LT.0) THEN
 	  YTOP = 2230.
	  YBOT = 200.
C		START AT 500 FOR HP PRINTER AT VAUGHN
	  XSTR = 500.
 	  XEND = 3000.
	ELSE
	  YTOP = GY2
...
C
	  ...
	  YHI = YLOW + .9*PRANGE	  
	  IF(ITERM.LT.0) THEN
C	    CALL MGOSETLOC(400.,YLOW,XEND,YHI)
C		500 for HP printer at Vaughn
	    CALL MGOSETLOC(XSTR,YLOW,XEND,YHI)
	  ELSE
...
C	      CALL MGOGRELOCATE(400.,50.)                      ! hardcopy
	      CALL MGOGRELOCATE(XSTR,50.)                      ! hardcopy
	    ENDIF
