...
C
	...
	PFC = 1.
	PFCX = PFC
	IF(ITERM.EQ.7) PFC = .38
	IF(ITERM.EQ.7) PFCX = .36
 100    CONTINUE 
	CALL MGOINIT
...
C
	...
	ELSE
	  XSTART = PFCX*100.
	  XEND = PFCX*880.
	  CALL MGOSETLOC(XSTART,PFC*80.,XEND,PFC*750.)
...
C
	...
	PFC = 1.
	PFCX = PFC
	IF(ITERMSV.EQ.7) PFC = .3
	IF(ITERMSV.EQ.7) PFCX = .38
 100    CONTINUE 
	CALL MGOINIT
	CALL MGOSETUP(ITERMT)
	CALL MGOERASE
