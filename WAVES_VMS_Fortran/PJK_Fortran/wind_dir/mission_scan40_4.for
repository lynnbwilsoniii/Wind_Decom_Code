...
C 1012	format(2X,'TIME OF CHANGE',4X,'DRV  RX  RY  RZ  VX  VY  VZ  DRV V' )
	...
	OPEN(UNIT=75,FILE='INSIDE5.RESULTS',STATUS='OLD',ACCESS='APPEND')
	OPEN(UNIT=77,FILE='INSIDE10.RESULTS',STATUS='OLD',ACCESS='APPEND')
 100	continue
...
C	print*,'open, TMCHANNEL,ok=',TMCHANNEL,ok
	if (ok.ne.1) stop 'Cannot open t/m channel in mission_scan40'
c
	scet = 0.
