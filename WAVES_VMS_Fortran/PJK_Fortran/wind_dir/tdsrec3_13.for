...
c		modified from tdsrec only to do part of a day
C
...
!
	common /nrblk/ nrem,NHRMN,NHRMNE
...
C        if(stream.eq.'offline') then
 10	  write(6,*)  'type hr,min to start and end, e.g. 0412,1200'
C	  read(5,3,err=10) iq, NHRMN,NHRMNE
	  read(5,*) NHRMN,NHRMNE
	  HH = NHRMNE/100
	  MM = MOD(NHRMNE,100)
	  type*,NHRMN,NHRMNE,HH,MM
C	endif
c
  5	format(q,a)
  3	format(q,i10)
...
C
	...
	PRINT*,'NHRMN,NHRMNE IN PLTMAX',NHRMN,NHRMNE
	NPTH1 = AMAX0(NPTH,1)
	NPTL1 = AMAX0(NPTL,1)
	NPTB1 = AMAX0(NPTB,1)
...
C
	PRANGE = (YTOP-YBOT)/6.
C	TSTART = AMIN1(HRHI(1),HRLO(1),HRB(1))
	  NHH = NHRMN/100
	  MM = MOD(NHRMN,100)
	TSTART  = FLOAT(NHH) + MM/60.
	  NHH = NHRMNE/100
	  MM = MOD(NHRMNE,100)
	TEND  = FLOAT(NHH) + MM/60.
C
	HRHI1 = HRHI(1)
	HRLO1 = HRLO(1)
	HRB1  = HRB(1)
	TRENORM = AMIN1(HRHI1,HRLO1,HRB1)
C	TEND = AMAX1(HRHI(NPTH1),HRLO(NPTL1),HRB(NPTB1))
...
C
	   NPTB3 = 0
	    DO N = 1,NPTB
	      FRQB(N) = 1000.*FRQB(N)
	      IF(HRB(N).GE.TSTART.AND.HRB(N).LE.TEND) NPTB3 = NPTB3+1
	    ENDDO
	  CALL MGOTICKSIZE(0.,0.,0.,0.)  
  	  CALL MGOSETLIM(TSTART,0.,TEND,1300.)
c  	  CALL MGOSETLIM(-1.1,0.,24.,1300.)
c	  CALL MGOGRID(0)
	  CALL MGOCONNECT(HRB,FRQB,NPTB)
	  CALL MGOPOINTS(60.,1,HRB,FRQB,NPTB)
	  CALL MGOSETEXPAND(.5)
...
c	  WRITE(STR,1014) TRENORM,NEARLYHI,NEARLYLO,NEARLYB
1014	  FORMAT('1st data',f7.2,' hours, no.evts,hi,lo,b',3i5)
c	  CALL MGOLABEL(54,STR)
	  CALL MGOBOX(1,2)
	  CALL MGOSETEXPAND(.5)
	  CALL MGOYLABEL(2,'Hz')
	  CALL MGOSETEXPAND(1.)
...
C  	  CALL MGOSETLIM(-1.1,0.,24.,130.)
c	  CALL MGOGRID(0)
	  CALL MGOCONNECT(HRB,TDSB,NPTB)
	  CALL MGOPOINTS(80.,1,HRB,TDSB,NPTB)
	  CALL MGOSETEXPAND(.5)
	  CALL MGOBOX(0,2)
...
C
	  ...
	  DO N = 1,NPTL
	    ...
	    IF(HRLO(N).GE.TSTART.AND.HRLO(N).LE.TEND) NPTL3 = NPTL3+1
	  ENDDO
...
C
	  CALL MGOTICKSIZE(0.,0.,0.,0.)  
  	  CALL MGOSETLIM(TSTART,0.,TEND,1600.)
C  	  CALL MGOSETLIM(-1.1,0.,24.,1600.)
c	  CALL MGOGRID(0)
	  CALL MGOCONNECT(HRLO,FRQLO,NPTL)
	  CALL MGOPOINTS(60.,1,HRLO,FRQLO,NPTL)
	  CALL MGOSETEXPAND(.5)
	  CALL MGOBOX(0,2)
	  CALL MGOSETEXPAND(.5)
	  CALL MGOYLABEL(2,'Hz')
...
c	  CALL MGOGRID(0)
	  ...
	  CALL MGOPOINTS(80.,1,HRLO,FRQLO,NPTL)
	  CALL MGOSETEXPAND(.5)
	  CALL MGOBOX(0,2)
	  CALL MGOSETEXPAND(.5)
	  CALL MGOYLABEL(7,'T/M NO.')
...
C
	  NPTH3 = 0
	  DO N = 1,NPTH
	      IF(HRHI(N).GE.TSTART.AND.HRHI(N).LE.TEND) NPTH3 = NPTH3+1
	  ENDDO
C	  CALL MGOTICKSIZE(0.,0.,5.6,28.)  
	  CALL MGOTICKSIZE(0.,0.,0.,0.)  
  	  CALL MGOSETLIM(TSTART,0.,TEND,40.)
C  	  CALL MGOSETLIM(-1.1,0.,24.,40.)
c	  CALL MGOGRID(0)
...
c	  CALL MGOGRID(1)
	  ...
	  CALL MGOPOINTS(80.,1,HRHI,FRQHI,NPTH)
	  CALL MGOSETEXPAND(.5)
	  CALL MGOBOX(0,2)
...
c	  CALL MGOGRID(1)
	  ...
	  CALL MGOPOINTS(60.,1,HRHI,TDSHI,NPTH)
	    do i = 1,npth
	      ihivpm = tdshi(i)+128
...
C	  TITLE1(6) = PA(NCH,INPUT+1)
	  ...
	  CALL MGOSETEXPAND(.8)
C
	  CALL MGOPLOTID('TDS MAX','[KELLOGG.WIND]TDSREC3')
	  CALL MGOSETEXPAND(1.)
C	 ENDDO
	  CALL MGOSETEXPAND(.6)
...
!
	...
	character*(*)	stream
	common /nrblk/ nrem,NHRMN,NHRMNE
	integer*4	iq,NREM,NHRMN,NHRMNE
