...
C
C	THIS IS MISSION_SCAN28, TO PLOT FFT AMPLITUDES, LOOKING FOR SHOCK
C	ENHANCEMENTS.  FFTH IS, I THINK, ALWAYS ELECTRIC.  FFTM ?  FFTL IS 
C	MAINLY MAGNETIC ?
C	not done yet
C
...
C
	COMMON /PLOTSTUFF/ YYYYT,MONT,DDT,SCETI4,STARTIME,ENDTIME
	character*80	stream
c	CHARACTER*80	YYYYMMDD
	...
	integer*4	yyyy,mon,dd,hh,mm,ss,dds,yyyyt,mont,ddt
C
...
C 1012	format(2X,'TIME OF CHANGE',4X,'DRV  RX  RY  RZ  VX  VY  VZ  DRV V' )
	...
	call w_ur8_to_ymd(scet8,yyyyt,mont,ddt,hh,mm,ss,ms)
	PRINT*,'YMD',YYYYT,MONT,DDT,HH,MM
	WRITE(FILEDATE,31) YYYYT,MONT,DDT
 31	FORMAT(I4.4,'/',I2.2,'/',I2.2)
	OPEN(UNIT=90,FILE='MAKEFILE28.RESULTS',STATUS='OLD',ACCESS='APPEND')
	NDO = 0
	PTIME = SCETSTR
 	AMP = SCETEND
