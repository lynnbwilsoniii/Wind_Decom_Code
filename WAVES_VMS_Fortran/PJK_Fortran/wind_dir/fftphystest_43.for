...
c******************
	     if(newold.eq.1.and.fft_channel.lt.7) go to 100
	     iw = fft_channel-6
...
C	     TITLE(iw) = PA(IQ)
 ...
 1001	   FORMAT('CH',I2,'  ',A4)
	   ...
	   item = 'DPU_MF'
	   ok = w_item_r4(ch, item, DPUTIME, 1, return_size)
	  TYPE*,'R4 DPU RETURN SIZE',RETURN_SIZE
	   if (.not. ok) type *, 'cannot get R4 item ', item, ', ok=', ok
	   item = 'DPU_MF'
	   ok = w_item_I4(ch, item, DPUTIME, 1, return_size)
	  TYPE*,'I4 DPU RETURN SIZE',RETURN_SIZE
	   if (.not. ok) type *, 'cannot get I4 item ', item, ', ok=', ok
C
...
c	   if (.not. ok) then
c	      iraw = 1
Cc	      type *, 'cannot get item ', item, ', ok=', ok
...
c*************** Cathie check*********
C	ncount = ncount+1
C	write(77,*) ncount,fft_channel,iq,(dbspec(n),n=1,513)
...
C
	IF(TDS_CHANNEL.EQ.1) THEN
	  WRITE(81,1002) SCET,FFT_CHANNEL,EVTIME,DPUTIME,TPOWER
	ENDIF
	IF(TDS_CHANNEL.EQ.2) THEN
	  WRITE(82,1002) SCET,FFT_CHANNEL,EVTIME,DPUTIME,TPOWER
	ENDIF
 1002	FORMAT(2I8,I4,F9.3,F9.3,E12.3)
C	
...
C
	  IF(IW.EQ.NWNDOW) THEN
	    CALL MGOSETEXPAND(.8)
	    CALL MGOPLOTID('FFT TEST','SHPLOT')
	    CALL MGOSETEXPAND(1.)
	  ENDIF
	ENDDO
C
...
C
	CALL MGOSETEXPAND(.7)
	CALL MGOPLOTID('[kellogg.wind]','FFTGRAY2')
	CALL MGOSETEXPAND(1.)
	IF(ITERM.LT.0) THEN
	  CALL MGOPRNTPLOT(NVEC)
	  PRINT*,' NO. VECTORS PLOTTED',NVEC
