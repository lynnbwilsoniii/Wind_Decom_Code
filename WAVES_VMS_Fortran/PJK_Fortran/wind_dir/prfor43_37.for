...
C
	COMMON /HISTBLK/ NHIST(256,5),NPOS(5),NEG(5)
	COMMON /HEADBLK/ FILE
...
C
	DO NW = 1,3
	  ...
	  IF(NW.EQ.1) THEN
	    NWU = 2
	    IF((NPOS(1)+NEG(1)).GT.(NPOS(2)+NEG(2))) NWU = 1
	  ENDIF
	  IF(NW.EQ.2) THEN
	    NWU = 4
	    IF((NPOS(3)+NEG(3)).GT.(NPOS(4)+NEG(4))) NWU = 3
	  ENDIF
	  IF(NW.EQ.3) NWU = 5
	  YMAX = 0.
	  DO N = 1,256
...
C
	...
	ENDDO
	CALL MGOSETEXPAND(.6)
	CALL MGOPLOTID('TDS HISTO','[KELLOGG.WIND]TDSREC')
	CALL MGOSETEXPAND(.5)
