...
C
C	This routine gets raw TDS data for the event found in the calling
C		program, and, according to IPRC, corrects it for gain and
C		calibration, and return VDATA = volts at preamp input, and 
C		spect = signal in dB above 1 V**2/Hz
C
C	FIXBADTDS was added about 1400 on 8 Oct 1995
C	9 MAY 2007, A PRINT STATEMENT FOR PWR = 0. WAS REMOVED, TOO MUCH
C	On Sep 15 2006, changed to call FIXBADTDS only for IRX = 1,2
...
C	FIXBADTDS6 was written in the fall of 1997, and works fairly well
...
C
	...
	ENDIF
C
	IF(IPRC.EQ.4.AND.IRX.LE.2)  THEN
...
C			FIRST EDITION  --  VERY IMPORTANT
C
...
C
	...
	DO IK = 3,2048,2
	    ...
	    ELSE
C		print*,'tds_phys',data(ik),data(ik+1),itemp,irx
	    ENDIF
	ENDDO
C
...
C		ABOUT 3 HZ
C
		IF(IRX.GE.7) FFTMIN = AMAX1(3.3,FFTMIN)
C
	print*,'in tds_phys, freqmin,max ',fftmin,fftmax
	     DO IK = 3,2048,2
	        FREQ = SPS*(IK-1)/4096.
