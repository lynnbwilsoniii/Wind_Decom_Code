...
C
	OPEN(UNIT=8,FILE='FOR008.DAT',TYPE='OLD',READONLY)
C	OPEN(UNIT=8,FILE='FOR008.DAT;2',TYPE='OLD',READONLY)
	XMIN = 0.
	XMAX = 0.
	YMIN = 0.
	YMAX = 0.
...
C
  20	CONTINUE
	CALL OSCAMG(5)
C	CALL OSCAMG(6)
	TYPE*,'STOP JUST AFTER 20 CONTINUE'
	READ(5,107) DISPOSE
	IF(1) STOP
C
...
C	COM(NCOM) = 'END'
	...
	T1 = SECNDS(0.0)
 123	T2 = SECNDS(T1)
	IF(T2.LT.5.) GO TO 123
C	IF(IMRP.EQ.5) COM(NCOM) = 'INPUT SHZMGO.MGO5'
	IF(IMRP.EQ.5) COM(NCOM) = 'INPUT SHZMGO.MGO6'
	CALL MONGO(NCOM,COM,200,10,PLT)
	T1 = SECNDS(0.0)
 124	T2 = SECNDS(T1)
	IF(T2.LT.5.) GO TO 124
	READ(5,107) DISPOSE
 107	FORMAT(A)
C  120	STOP
	END
