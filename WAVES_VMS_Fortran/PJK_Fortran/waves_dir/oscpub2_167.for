	PROGRAM OSCPUB2
C
C	PLOT RESULTS OF PAIRS OF OSCARS RUNS WHICH HAVE BEEN STORED IN FOR008
...
C	THE SECOND OF EACH PAIR IS DARKER, SO SHOULD BE ION ACOUSIC HERE
C
...
C	ITERM = -1
	...
	LASTFILE = 2
	LASTFILE = 6
	IF(LASTFILE.GT.2.AND.ITERM.EQ.-1) ITERM = -2
	IF(LASTFILE.GT.2.AND.ITERM.EQ.-3) ITERM = -4
c	IFIG = 5		! modes 10 Rsun
	IFIG = 3		! modes Cs>VA
C	IFIG = 4		! modes VA>Cs
c	IFIG = 0
C
...
C
C	THIS PLOTS LASTFILE BOXES.  BUT THE BOXES ARE NOT NUMBERED BY
...
C 	the first plot is heavier line
C
	COMMON /PPAK/ XMIN,XMAX,DELX,YMIN,YMAX,DELY,IFIG,RAD(10)
	COMMON /PLTBLK/ PLT(750,10)
	COMMON /PLTR/ WRXMIN(10),WRXMAX(10),WRYMIN(10),WRYMAX(10)
...
C		for terminal, order seems to be x1,y1,x2,y2
	        CALL MGOSETLOC(100.,100.,950.,700.)
	    ELSE
		RATIO = 1.	  
	        CALL MGOSETLOC(100.,150.,950.,700.)
	    ENDIF
	  ELSE
	    RATIO = 1.
	    IF(LASTFILE.LE.2) RATIO = 160./98.5	   ! RATIO X TO Y WITH 2 WINDOWS
	    CALL MGOSETLOC(500.,450.,2800.,2000.)
	  ENDIF
	  NWX = LASTFILE/2		! NUMBER OF BOXES IN X DIRECTION
C
...
C
	write(39,*) 'page,gx1,gx2',gx1,gx2,gy1,gy2
	  ...
	  DO N = 1,LASTBOX+1
	    NWX = LASTBOX/2	
	    NBOXI = (N+1)/2		!  NBOXI = 1,1,2,2,3,ETC.
	    NBOXM = MOD(N-1,NWX)	!  0 IS IMAG, 1 IS REAL PART
	    NFILEY = (N-1)/NWX     
...
C
	IF(IPLOT.EQ.1) THEN
	  NWX = LASTFILE/2		! NUMBER OF BOXES IN X DIRFECTGION
	  NBOXI = (NFILE+1)/2		! NBOXI = 1,2,3,ETC., NO OF 
	  NBOXR = NBOXI + NWX		!  
	  NFILEM = MOD((NFILE-1),2)     !  NFILEM = 0 OR 1, 0 = SETUP, AND
C							THICK LINE	
C 
	  write(39,*) 'nfile,NWX,NBOXI,nfilem',NFILE,NWX,NBOXI,nfilem
	  IF(NFILEM.EQ.0) THEN
...
C
	  ...
	  DO NN = 1,NLINES
	    ...
	    YY(NN) = PLT(NN,6)
	  ENDDO     
	  IF(NFILEM.EQ.0) CALL MGOSETLWEIGHT(3)
