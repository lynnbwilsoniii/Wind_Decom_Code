...
C
     ...
     1 CM3,CN3,CM4,CN4,T,TM(100),TN(100),TV(100),TW(100),
     2 ST,SU,SV,SW,GJIM,GJIN,GYKM,GYKN,
...
C
 ...
 10	FORMAT (' WARNING (BESSEL) -- IFR, IBM, IBN',3I10)
      TM(K+1)=-SGN*TM(K)*X/FLOAT(IFR*(IBM-IFR))
      TN(K+1)=-SGN*TN(K)*X/FLOAT(IFR*(IBN-IFR))
      IFR=IFR+1
      T2=CABS(TN(K+1))
C**EPSI(2)=1.E-18
...
C**EPSI(2)=1.E-18
     ...
     1   .AND. T5 .GE. CABS(ZT4)) STOP 775
      IF(CABS(ZT3) .GE. EPSI(2) .AND. T5 .LT. CABS(ZT4)) K=K-1
      S1=TM(K)
      S2=TN(K)
      S3=TV(K)
      S4=TW(K)
...
C********************************************************************
 ...
 5091 FORMAT(// 40H RUN ERROR IN BESSEL FUNCTION SUBROUTINE)
      WRITE(7,5092) PHI,CHI,ORD,OPT1,OPT2
 5092 FORMAT(1H0,5HPHI =,F15.8,4X,5HCHI =,F15.8,4X,5HORD =,F6.1,4X,
     1 6HOPT1 =,F6.1,4X,6HOPT2 =,F6.1)
