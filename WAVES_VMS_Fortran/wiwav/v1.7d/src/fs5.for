! fs5.for - wind/waves file chooser #5.  This chooser is closely coupled with
! chooser #2 because of shared routines, commons, and terminal screen
! interface.

!------------------------------------------------------------------------------
! Entry w_c5_line_data is called by the scroll-window manager to place the si-th
! completely formatted line item in global buffer s(1).str.
! Entry get_file_list is called to initialize the file list with all the
! file names from the target directory.
!------------------------------------------------------------------------------
	integer*4	function	w_c5_line_data(si)
	implicit	none
	...
	integer*4	k2len
	integer*4	flags
	integer*4	w_c5_get_line_list		! an entry point
	integer*4	w_c5_reset_line_list		! an entry point
	integer*4	ios
	character*80	msg
	integer*4	first_time /1/
	integer*4	first_time2 /1/
	integer*4	w_c5_build_file_sublist		! a function
	integer*4	w_c5_set_state
	integer*4	w_c5_get_state
	integer*4	yr,mn
...
!	call lib$wait(0.15)
	...
	if (state .eq. w_fs5_mode_dom) then
	   sim1 = abs(si) - 1
	...
	1	sim1, files(si),
	1	primary,bc
	   else if (si .eq. s(y).idx) then	! current line, show reverse
	      write(s(y).str,1) 
	1	bc,reverse,
	1	sim1, files(si),
	1	primary,bc
	   else if (si .ge. 1 .and. si .le. s(y).idz) then ! not current, normal
	      write(s(y).str,1) 
	1	bc,primary,
	1	sim1,files(si),
	1	primary,bc
	   else if (si .gt. s(y).idz) then		! blank line
	      write(s(y).str,2) 
	1	bc,primary,
	1	' ',
	1	primary,bc
	...
	else if (state .eq. w_fs5_mode_year .or.
	1        state .eq. w_fs5_mode_month) then
	   sim1 = si
	   if (state .eq. w_fs5_mode_month) sim1 = abs(si) - 1
	   if (si .lt. 0) then
	      ! bold current line (display may not be current)
	      si = - si
	      write(s(y).str, 1) bc, bright, sim1, dirs(si), primary, bc
	      !
	   else if (si .eq. s(y).idx) then
	      ! current line, show reverse (display is current)
	      write(s(y).str, 1) bc, reverse, sim1, dirs(si), primary, bc
	      !
	   else if (si .ge. 1 .and. si .le. s(y).idz) then
	      ! not current, normal
	      write(s(y).str, 1) bc, primary, sim1, dirs(si), primary, bc
	      !
	   else if (si .gt. s(y).idz) then
	      ! blank line
	      write(s(y).str, 2) bc, primary, blank, primary, bc
	      !
	   else
	      call w_c5_err('index for directory list out of range')
	      s(y).str = 'jon '
	   end if
	else
	   call w_c5_err('w_c5_line_data: unknown state')
	end if
	s(y).l_str = 7 + 6 + 70 + 7 
 ...
 2	format(a3,a4,            a01,75x,a4,a3)
	...
	!----------------------------------------------------------------------
	entry	w_c5_reset_line_list()
	w_c5_reset_line_list = 1
	...
	!----------------------------------------------------------------------
	entry	w_c5_get_line_list()
	w_c5_get_line_list = 1
	files(1) = '*NONE*'
	if (first_time2 .eq. 1) then
	   call w_c5_init_file_list()	! this should be the only call
	   first_time2 = 0
	end if
	if (state .eq. w_fs5_mode_dom) then
	   ok = w_c5_build_file_sublist()
	else
	   if (vfl.i .eq. 0) then
	      state = w_fs5_mode_year
	   else
	      if (vfl.vy( vfl.i ).i .eq. 0) state = w_fs5_mode_month
	   end if
	   if (state .eq. w_fs5_mode_year) then
	      ...
	      s(y).xr  = 0 ! an absolute terminal screen row number
	      s(y).idz = n
	   else if (state .eq. w_fs5_mode_month) then
	      ...
	      s(y).xr  = 0 ! an absolute terminal screen row number
	      s(y).idz = n ! data index of last available line item
	   else
	      call w_c5_msg('Invalid state for virtual directory')
	   end if
	end if
...
!------------------------------------------------------------------------------
	integer*4	function	w_c5_set_cur_dir()
	implicit	none
	include		'termvideo_def.for'
	include		'scr_manager_def.for'
	include		'file_select2_def.for'
	...
	integer*4	i,j
	character*64	new
	integer*4	w_c5_show_file_display	! an entry point
	...
	if (vfl.i .eq. 0) then
	   ...
	   state = w_fs5_mode_month
	   c4 = dirs(s(1).idx)
	   call w_c5_msg('selected year '//c4)
	   ! match the year
	   i = 1
	...
	1           vfl.vy(i).year .ne. c4)
	   ...
	   if (i.gt.max_years_of_mission) then
	      call w_c5_err('cannot set current vdir, sit.1')
	   end if
	   vfl.i = i
	   vfl.vy(i).i = 0
 	   call w_c5_get_line_list()
!xxxxx	   call scr_mgr_show_one_scroll(s(1))
	   call w_c5_update_top_status_line()
	   w_c5_set_cur_dir = 0
	   return
...
!xxxxx	   call scr_mgr_show_one_scroll(s(1))
	   ...
	   state = w_fs5_mode_dom
	   yr = vfl.i
	   j = 1
	   c3 = dirs(s(1).idx)
	   call w_c5_msg('selected month '//c3)
	   do while(c3 .ne. month_list(j) .and. j .le. 12)
	      j = j + 1
	   end do
	   if (j .gt. 13) then
	      call w_c5_err('cannot set current vdir, sit.2')
	      return
	   end if
	   vfl.vy(yr).i = j
	   call w_c5_update_top_status_line()
...
!------------------------------------------------------------------------------
	integer*4	function	w_c5_line_list_after(x) !,si)
	implicit	none
	include		'termvideo_def.for'
	include		'scr_manager_def.for'
	include		'file_select2_def.for'
...
!	integer*4	si
	...
	integer*4	ok
	integer*4	w_c5_set_cur_dir
	integer*4	w_c5_file_list_after

	w_c5_line_list_after = 0
	keyname = x.retkey

	if (state .eq. w_fs5_mode_dom) then
	   ok = w_c5_file_list_after(x)
	else
! if (state .eq. w_fs5_mode_year .or.
!	1        state .eq. w_fs5_mode_month) then
	   if ( (keyname .eq. 'DO') .or.
	...
	w_c5_line_list_after = ok
	return
	end

	options/extend_source
!------------------------------------------------------------------------------
! Calls w_c5_set_cur_dir to move the current file search/display directory
! up one level in the heirarchy.
!------------------------------------------------------------------------------
	integer*4	function	w_c5_up_one_dir_lev_routine()
	implicit	none
	include		'termvideo_def.for'
	include		'scr_manager_def.for'
	include		'file_select2_def.for'

	w_c5_up_one_dir_lev_routine = 0

	call w_c5_err('"Hot Key" for up one directory not implemented')

	w_c5_up_one_dir_lev_routine = 1
	return
	end

!------------------------------------------------------------------------------
...
! keystroke used to exit the scroll region.
!------------------------------------------------------------------------------
	integer*4	function	w_c5_file_list_after(x) !,si)
	implicit	none
	include		'termvideo_def.for'
	include		'scr_manager_def.for'
	include		'file_select2_def.for'
...
!	integer*4	si
	...
	w_c5_file_list_after = 0
	keyname = x.retkey

	if (keyname .eq. 'CNTL/Z') then
	   w_c5_file_list_after = 1
	   result_file = ' '
	...
	1    (keyname .eq. 'PF4')
	1) then
	   if (s(1).idx .eq. 1) then
	      ! user picked [-]
!	w_c5_file_list_after = 1 ! success, call "before" for next field
	w_c5_file_list_after = 0 ! fail, remain in "current" field
	      state = w_fs5_mode_month
	      call w_c5_get_line_list()
	s(1).xr = 0
	s(1).idx = 1
...
!xxxxx	      call scr_mgr_show_one_scroll(s(1))
	      ...
	      k = s(1).idx
	      read(files(k)(1:2),'(i2.2)',iostat=o) j
	      call ch_number_of_month(c2,files(k)(4:6))
...
!	   call w_c5_reset_file_list()
 	   call w_c5_reset_line_list()
	...
	return
! 10	continue
!	call w_c5_err('No current directory is specified.')
!	return
! 20	continue
!	call w_c5_err('Zero length directory specified.')
!	return
	end

!------------------------------------------------------------------------------
	integer*4	function	w_c5_build_file_sublist()
	implicit	none
	include		'termvideo_def.for'
	include		'scr_manager_def.for'
	include		'file_select2_def.for'
	...
	s(1).idw = 1
	s(1).idz = n
	write(c4,'(i4)',iostat=o) (n - 1)
	call w_c5_msg(c4//' days found for month '//month_list(k))

	w_c5_build_file_sublist = 1

	return
	end

!------------------------------------------------------------------------------
	integer*4	function	w_c5_reset_vfl(x)
	implicit	none
	include		'scr_manager_def.for'
	include		'file_select2_def.for'
	integer*4	x
...

!------------------------------------------------------------------------------
	integer*4	function	w_c5_init_file_list()
	implicit	none
	include		'termvideo_def.for'
	include		'scr_manager_def.for'
	include		'file_select2_def.for'
	integer*4	ok
	integer*4	i,j,k,m,n,o,x
	integer*4	k2len
	...
	integer*4	flags
	integer*4	w_c5_get_file_list		! an entry point
	...
	character	c4*4, c2*2
	integer*4	i_mon, i_dom
	record /virtual_year/ tmp_vy
	logical*4	sorted

	w_c5_init_file_list = 0

	call w_c5_msg('Getting file list, please wait...')
	! assume all files are in a single directory
	file_spec = 'wind_data:wi_lz_wav_*v%%.dat'
	k = k2len(file_spec)

	call w_c5_reset_vfl(1)
...
!	call lib$wait(0.2)
	      ...
	      if (c4 .ne. vfl.vy(x).year) then
	         ...
	         m = 0
	         j = 1
	         do while (j .le. max_years_of_mission)
	            if (c4 .eq. vfl.vy(j).year) m = j
	            if (vfl.vy(j).year .gt. ' ') x = j
	            j = j + 1
	         end do
	         if (m .ne. 0) then
	            x = m
...
!	call w_c5_msg('Year '//c4//' ver:'//c2//' mndy: '//file(i+4:i+7))
	      else if (c2 .gt. vfl.vy(x).vm(i_mon).dayver(i_dom)) then
	         vfl.vy(x).vm(i_mon).dayver(i_dom) = c2
	      end if
	   else if (ok .eq. no_such_file) then
	      call w_c5_err('No files found, please check selection criteria.')
	      files(1) = '*NONE*'
	   else if (ok .eq. file_not_found) then
	      call w_c5_err('No files found in this directory.')
	      files(1) = '*NONE*'
	...
	if (ok .eq. rms_no_more_files) then
	   ...
	   call w_c5_msg(c4//' files found.')
	end if

	! files will usually be sorted or at least nearly sorted, so
	! do a simple sort on this short list
	j = 0
	x = 0
	do while (j .le. max_years_of_mission)
	   if (vfl.vy(j).year .gt. ' ') x = j
	...
	do while (.not. sorted)
	   ...
	   do i=1,x-1
	      j = i + 1
	      if (vfl.vy(i).year .gt. vfl.vy(j).year) then
	         sorted = .false.
	         tmp_vy    = vfl.vy(i)
	         vfl.vy(i) = vfl.vy(j)
	         vfl.vy(j) = tmp_vy
	   ...
	   if (k .gt. 100) then
	      sorted = .true.
	      call w_c5_msg(' Error sorting files by year.')
	   end if
	end do

	w_c5_init_file_list = 1
	return
 11	format('Error getting file #', i3, ', ok=', z8.8, ', rms=',z8.8)
 20	continue
	call w_c5_err('Unexpected file name: '//file(1:60))
	return
 30	continue
	call w_c5_err('Err getting mon./day: '//file(1:60))
	return
 32	continue
	call w_c5_err('Range err mon./day: '//file(1:60))
	return
 40	continue
	call w_c5_err('Overflow: Additional files are not listed.')
	return
	end

!------------------------------------------------------------------------------
...
!------------------------------------------------------------------------------
	...
	call w_c5_reset_line_list()
!	call w_c5_reset_dir_list()
	call w_c5_reset_vfl(0)

	call putstr(soft_reset)
	call w_c5_setup_screen_fields()
	call w_c5_build_screen()
	call w_c5_refresh()
	ok = manage_screen(g,on,w_c5_err)
	call w_c5_post_screen()
	if (ok) then
...
!------------------------------------------------------------------------------
	integer*4	function	w_c5_post_screen()
	implicit	none
	include		'($lnmdef)'
	include		'termvideo_def.for'
	include		'scr_manager_def.for'
...
!------------------------------------------------------------------------------
	integer*4	function	w_c5_setup_screen_fields()
	implicit	none
	include		'termvideo_def.for'
	include		'scr_manager_def.for'
	include		'file_select2_def.for'
...
!	integer*4	get_search_list			! a function
	...
	integer*4	w_c5_err
	external	w_c5_err
	integer*4	w_c5_refresh
	external	w_c5_refresh
	integer*4	w_c5_exit
	external	w_c5_exit
	integer*4	w_c5_up
	external	w_c5_up
	integer*4	w_c5_down
	external	w_c5_down
	integer*4	w_c5_next_field
	external	w_c5_next_field
	integer*4	w_c5_prev_field
	external	w_c5_prev_field
	integer*4	w_c5_get_line_list
	external	w_c5_get_line_list
	integer*4	w_c5_line_data
	external	w_c5_line_data
	integer*4	w_c5_line_list_after
	external	w_c5_line_list_after
	integer*4	w_c5_set_object_pointer
	external	w_c5_set_object_pointer
	integer*4	w_c5_scroll_on_key_handler
	external	w_c5_scroll_on_key_handler
	integer*4	w_c5_do_routine
	external	w_c5_do_routine
	integer*4	w_c5_help_routine
	external	w_c5_help_routine
	integer*4	w_c5_up_one_dir_lev_routine
	external	w_c5_up_one_dir_lev_routine
	integer*4	w_c5_select_realtime
	external	w_c5_select_realtime
	integer*4	w_c5_select_lastfile
	external	w_c5_select_lastfile
	integer*4	w_c5_select_by_prompt
	external	w_c5_select_by_prompt
	integer*4	w_c5_size_toggle
	external	w_c5_size_toggle
	integer*4	w_c5_display_last_file_used
	external	w_c5_display_last_file_used
	integer*4	use_virtual_dir
	external	use_virtual_dir
	integer*4	i_g, i_f, i_s

	w_c5_setup_screen_fields = 0
...
!	if (ok .ne. 1) then
!	   call w_c5_err('Error getting search list: '//default_dir)
!	end if
...
!	current = search_list(1)
	...
	g(i_g).before = %loc(w_c5_get_line_list)
	g(i_g).proc = %loc(scroll_this)
	g(i_g).after = %loc(w_c5_line_list_after)
	g(i_g).obj  = %loc(s(i_s))
	g(i_g).errhdlr = %loc(w_c5_err)

	s(i_s).flags    = top_is_sticky .or. bottom_is_sticky
	s(i_s).r1	= 3
	s(i_s).r2	= 21
	s(i_s).gtscrldt	= %loc(w_c5_line_data)
	s(i_s).on_key	= %loc(w_c5_scroll_on_key_handler)
	s(i_s).retkey   = ' '
	s(i_s).xr       = 0
	s(i_s).idw      = 0
	s(i_s).idx      = 0
...
!	i_s = 2
!	g(i_g).type = scroll_this_type
!	g(i_g).before = %loc(w_c5_get_file_list)
!	g(i_g).proc = %loc(scroll_this)
!	g(i_g).after = %loc(w_c5_file_list_after)
!	g(i_g).obj  = %loc(s(i_s))
!	g(i_g).errhdlr = %loc(w_c5_err)
!	s(i_s).r1	= 10
!	s(i_s).r2	= 21
!	s(i_s).idz	= 35
!	s(i_s).gtscrldt	= %loc(w_c5_file_data)
!	s(i_s).on_key	= %loc(w_c5_scroll_on_key_handler)
!	s(i_s).retkey   = ' '
!	s(i_s).xr       = 0
!	s(i_s).idw      = 0
...
!
	...
	on(1).key = 'CNTL/W'
	on(1).routine = %loc(w_c5_refresh)
	on(2).key = 'CNTL/Z'
	on(2).routine = %loc(w_c5_exit)
	on(3).key = 'UP'
	on(3).routine = %loc(w_c5_up)
	on(4).key = 'DOWN'
	on(4).routine = %loc(w_c5_down)
	on(5).key = 'CNTL/B'
	on(5).routine = %loc(w_c5_prev_field)
	on(6).key = 'CNTL/I'
	on(6).routine = %loc(w_c5_next_field)
	on(7).key = 'CNTL/M'
	on(7).routine = %loc(w_c5_exit)
	on(8).key = 'NEXT'
	on(8).routine = %loc(w_c5_set_object_pointer)
	on(9).key = 'PREV'
	on(9).routine = %loc(w_c5_set_object_pointer)
	on(10).key = 'DO'
	on(10).routine = %loc(w_c5_do_routine)
	on(11).key = 'SELECT'
	on(11).routine = %loc(w_c5_do_routine)
	on(12).key = 'HELP'
	on(12).routine = %loc(w_c5_help_routine)
	on(13).key = 'H'
	on(13).routine = %loc(w_c5_help_routine)
	on(14).key = 'h'
	on(14).routine = %loc(w_c5_help_routine)
	on(15).key = '?'
	on(15).routine = %loc(w_c5_help_routine)
	on(16).key = 'U'
	on(16).routine = %loc(w_c5_up_one_dir_lev_routine)
	on(17).key = 'u'
	on(17).routine = %loc(w_c5_up_one_dir_lev_routine)
	on(18).key = 'R'
	on(18).routine = %loc(w_c5_select_realtime)
	on(19).key = 'r'
	on(19).routine = %loc(w_c5_select_realtime)
	on(20).key = 'L'
	on(20).routine = %loc(w_c5_select_lastfile)
	on(21).key = 'l'
	on(21).routine = %loc(w_c5_select_lastfile)
	on(22).key = 'P'
	on(22).routine = %loc(w_c5_select_by_prompt)
	on(23).key = 'p'
	on(23).routine = %loc(w_c5_select_by_prompt)
	on(24).key = 'I'
	on(24).routine = %loc(w_c5_size_toggle)
	on(25).key = 'i'
	on(25).routine = %loc(w_c5_size_toggle)
	on(26).key = 'K'
	on(26).routine = %loc(w_c5_display_last_file_used)
	on(27).key = 'k'
	on(27).routine = %loc(w_c5_display_last_file_used)
	on(28).key = 'V'
...
! those actually coded in routine w_c5_setup_screen_fields().
!------------------------------------------------------------------------------
	integer*4	function	w_c5_help_routine(xi)
	implicit	none
	include		'termvideo_def.for'
	include		'scr_manager_def.for'
	include		'file_select2_def.for'
	...
	call get_key(dummy)
	call w_c5_refresh(xi)

!	if (xi .eq. i_lf) then
!	   call w_c5_msg('Press RETURN to accept this file, CNTL/Z to quit.')
!	else if (xi .eq. i_sdir) then
!	   call w_c5_msg('Use arrows to move bar, press DO to change directory.')
!	else if (xi .eq. i_sdir) then
!	   call w_c5_msg('Use arrows to move bar, press DO to select file.')
!	end if
...
!------------------------------------------------------------------------------
	integer*4	function	w_c5_do_routine(xi)
	implicit	none
	include		'scr_manager_def.for'
	include		'file_select2_def.for'
	integer*4	xi
	...
	1 call w_c5_msg('inside w_c5_do_routine')
...
!------------------------------------------------------------------------------
	integer*4	function	w_c5_goto_bottom()
	implicit	none
	include		'termvideo_def.for'
	include		'scr_manager_def.for'
...
!------------------------------------------------------------------------------
	integer*4	function	w_c5_update_top_status_line() 
	implicit	none
	include		'termvideo_def.for'
	include		'scr_manager_def.for'
	include		'file_select2_def.for'
...
!	if (vfl.i .eq. 0) then
	if (state .eq. w_fs5_mode_year) then
	   line = ' Current Directory WIND_DATA:'
!	else if (vfl.vy(vfl.i).i .eq. 0) then
	else if (state .eq. w_fs5_mode_month) then
	   line = ' Current Directory WIND_DATA:['//vfl.vy(vfl.i).year//']'
	else
	   line = ' Current Directory WIND_DATA:['
	1	//vfl.vy(vfl.i).year//'.'
...
!------------------------------------------------------------------------------
	integer*4	function	w_c5_update_bottom_status_line() 
	implicit	none
	include		'termvideo_def.for'
	include		'scr_manager_def.for'
	include		'file_select2_def.for'
	...
	w_c5_update_bottom_status_line = 1
	return
	end

!------------------------------------------------------------------------------
...
!------------------------------------------------------------------------------
	integer*4	function	w_c5_refresh()
	implicit	none
	include		'termvideo_def.for'
	include		'scr_manager_def.for'
	include		'file_select2_def.for'
!	integer*4	xi		! argument unused
	...
	call w_c5_update_top_status_line()
	call w_c5_update_bottom_status_line()
...
!------------------------------------------------------------------------------
	integer*4	function	w_c5_err(str)
	implicit	none
	include		'termvideo_def.for'
	include		'scr_manager_def.for'
	include		'file_select2_def.for'
...
!------------------------------------------------------------------------------
	integer*4	function	w_c5_msg(str)
	implicit	none
	include		'termvideo_def.for'
	include		'scr_manager_def.for'
	include		'file_select2_def.for'
...
!------------------------------------------------------------------------------
	integer*4	function	w_c5_next_field(xi)
	implicit	none
	include		'scr_manager_def.for'
	include		'file_select2_def.for'
	integer*4	xi
...
!------------------------------------------------------------------------------
	integer*4	function	w_c5_prev_field(xi)
	implicit	none
	include		'scr_manager_def.for'
	include		'file_select2_def.for'
	integer*4	xi
...

!------------------------------------------------------------------------------
! Calls w_c5_prev_field to move to previous field on screen.
!------------------------------------------------------------------------------
	integer*4	function	w_c5_up(xi)
	implicit	none
	include		'scr_manager_def.for'
	include		'file_select2_def.for'
	integer*4	xi
	call w_c5_prev_field(xi)
	w_c5_up = 1
	return
	end

!------------------------------------------------------------------------------
! Calls w_c5_next_field to move to next field on screen.
!------------------------------------------------------------------------------
	integer*4	function	w_c5_down(xi)
	implicit	none
	include		'scr_manager_def.for'
	include		'file_select2_def.for'
	integer*4	xi
	call w_c5_next_field(xi)
	w_c5_down = 1
	return
	end

...
!------------------------------------------------------------------------------
 	integer*4	function	w_c5_set_object_pointer(xi)
	implicit	none
	include		'scr_manager_def.for'
	include		'file_select2_def.for'
	integer*4	xi

	w_c5_set_object_pointer = 1
!xxxxxxxxxx
	if (g(xi).retkey .eq. 'NEXT') then
!	   if (s(xi).idx .eq. s(xi).idz) then
!	      call w_c5_err('...situation 1...')
!	      call lib$wait(5.0)
!	      w_c5_set_object_pointer = 0
!	   end if
...
!	      xi = i_sdir
!	   else
!	      call w_c5_err('Cannot set object pointer (A).')
!	   end if
...
!	      xi = i_sdir
!	   else
!	      call w_c5_err('Cannot set object pointer (B).')
!	   end if
	   ...
	   call w_c5_err('Cannot set object pointer (G).')
	end if
	return
	end

...
!------------------------------------------------------------------------------
	integer*4	function	w_c5_build_screen()
	implicit	none
	include		'termvideo_def.for'
	include		'scr_manager_def.for'
	include		'file_select2_def.for'
...
!------------------------------------------------------------------------------
	integer*4	function	w_c5_select_realtime()
	implicit	none
	include		'termvideo_def.for'
	include		'scr_manager_def.for'
	include		'file_select2_def.for'

	w_c5_select_realtime = 0
...
! Triggers the screen manager to return control to parent module.
!------------------------------------------------------------------------------
	integer*4	function	w_c5_select_lastfile()
	implicit	none
	include		'termvideo_def.for'
	include		'scr_manager_def.for'
	include		'file_select2_def.for'

	w_c5_select_lastfile = 0
...
!------------------------------------------------------------------------------
	integer*4	function	w_c5_display_last_file_used()
	implicit	none
	include		'termvideo_def.for'
	include		'scr_manager_def.for'
	include		'file_select2_def.for'
	...
	w_c5_display_last_file_used = 1
	if (last_file_used(1:1) .gt. ' ') then
	   k = k2len(last_file_used)
	   call w_c5_msg('Last file was: '//last_file_used(:k))
	else
	   call w_c5_msg('There is no previously used .WND file for this process.')
	end if
...
!------------------------------------------------------------------------------
	integer*4	function	w_c5_size_toggle(xi)
	implicit	none
	include		'termvideo_def.for'
	include		'scr_manager_def.for'
	include		'file_select2_def.for'
...
!------------------------------------------------------------------------------
	integer*4	function	w_c5_select_by_prompt()
	implicit	none
	include		'termvideo_def.for'
	include		'scr_manager_def.for'
	include		'file_select2_def.for'
	...
	w_c5_select_by_prompt = 0
	call putstr(prompt)
	read(5,'(q,a)',iostat=ios) i,result_file
	if (ios .ne. 0) then
	   write(err,'(i3.3)') ios
	   call w_c5_refresh()
	   call w_c5_err('Error entering file name, iostat='//err)
	   w_c5_select_by_prompt = 1
	else if (k2len(result_file) .lt. 1) then
	   call w_c5_msg('No file entered.')
	   w_c5_select_by_prompt = 1
	end if
...
!------------------------------------------------------------------------------
	integer*4	function	w_c5_scroll_on_key_handler(xi,keyname)
	implicit	none
	include		'termvideo_def.for'
	include		'scr_manager_def.for'
	include		'file_select2_def.for'
