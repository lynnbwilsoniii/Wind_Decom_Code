...
C	************************
	...
	CHARACTER*32 RAWDATFILE
      CHARACTER*4 YEARC,MONC,DAYC,HHC,MMC
...
C	END DATAFILE VARS
	...
	COMMON /PLTBLK/ IZCNT,IPROCESS,ZCROSS(2048),ZINT(2048),
     1   NDATA(2048),DATA(2050),SPECT(1025),TMDATA(2048),
     1   RVDATA(2048),RAWT(2048)
...
C
 ...
 1014		FORMAT(F12.3)
	       ...
	       CALL TDS_RPHYS(CH,IPROCESS,NDATA,DATA,SPECT,TMDATA,RVDATA)
...
C
	number_event = itemp
C
C	SET UP DATFILE/RAWDATFILE FOR DATE/TIME OF SAMPLE
C	**************************************
	YEARC=TOCHAR(YYYY)
	MONC=TOCHAR(MON)
	DAYC=TOCHAR(DD)
...
C       NAME DATFILE
     ...
     1 '_' // HHC(1:2) // MMC(1:2) // '-' //
     2 SSC(1:2) // '_CH' // CHC(1:2) // '.DAT'
C	OPEN DATFILE FOR WRITING
      OPEN(UNIT=77,FILE=DATFILE,STATUS='NEW')
C       NAME RAWDATFILE
      RAWDATFILE = YEARC // MONC(1:2) // DAYC(1:2) //
     1 '_' // HHC(1:2) // MMC(1:2) // '-' //
     2 SSC(1:2) // '_CH' // CHC(1:2) // '_TM_RAW.DAT'
C	OPEN RAWDATFILE FOR WRITING
      OPEN(UNIT=78,FILE=RAWDATFILE,STATUS='NEW')
C
C	WRITE HEADER INFO TO CORRECTED DATFILE
C	****************************
	WRITE(77,3011) 'FILE:',DATFILE
	WRITE(77,3012) 'SAMPLE TIME:',YYYY,'/',MON,'/',
     1 DD,' ',HH,':',MM,'.',SS,' UT'
...
C	WRITE HEADER INFO TO RAWDATFILE
C	****************************
	WRITE(78,5011) 'FILE:',RAWDATFILE
	WRITE(78,3012) 'SAMPLE TIME:',YYYY,'/',MON,'/',
     1 DD,' ',HH,':',MM,'.',SS,' UT'
	WRITE(78,3013) 'EVENT NO:',ITEMP
	WRITE(78,3015) 'CHANNEL:',TDS_CHANNEL
	WRITE(78,3016) 'SAMPLE RATE:',(.001*SPS),' kHZ'
	WRITE(78,3017) 'LP FILTER:',FILTER,' HZ'

C	*****************************************
C	RAWDATFILE OUTPUT
C	**************
	WRITE(78,5018) 'FORTRAN FORMATS'
	WRITE(78,5019) 'QUANTITY:','(T2,A9,T12,A4,T27,A9,',
     1 'T42,A9)'
      WRITE(78,5020) 'UNITS:','(T2,A6,T12,A1,T27,A2,',
     1 'T42,A1)'
      WRITE(78,5021) 'DATA:','(T2,I4.4,T12,E12.6,',
     1 'T27,E12.6)'
      WRITE(78,5022)'DATA POINTS:','2048'
	WRITE(78,5010) 'QUANTITY:','TIME','TELEMETRY','RAW VOLTS'
	WRITE(78,5099) 'UNITS:','s','TM','V'
      WRITE(78,5014) 'BEGIN!'
	DO ROC = 1,2048
 	RAWT(ROC) = 1000.*(ROC-1)/SPS 
	WRITE(78,5003) ROC,RAWT(ROC),TMDATA(ROC),RVDATA(ROC)
	ENDDO
      WRITE(78,5098) 'END!' 	  
C	DATFILE FORMATS
C	***************
 5003 FORMAT(T2,I4.4,T12,E12.6,T27,E12.6,T42,E12.6) !RAW
 5014 FORMAT(T2,A6)
 5018 FORMAT(T2,A15)
 5098 FORMAT(T2,A4)
 5019 FORMAT(T5,A9,T25,A21,A7)
 5020 FORMAT(T5,A6,T25,A21,A7)
 5021 FORMAT(T5,A5,T25,A19,A10,/)
 5022 FORMAT(T2,A20,T25,A4)
 5010 FORMAT(T2,A9,T12,A4,T27,A9,T42,A9) ! RAWLABEL
 5099 FORMAT(T2,A6,T12,A1,T27,A2,T42,A1) ! RAWUNITS
C	***************
...
C     1 A15,T87,A15) !LABEL
 ...
 5011 FORMAT(T2,A5,T25,A32,/) ! RDATFILENAME
 3012 FORMAT(T2,A12,T25,I4.4,A1,I2.2,A1,I2.2,A1,I2.2,A1,
     1 I2.2,A1,I2.2,A3) ! DATE/TIME
...
C	PLOT FREQUENCY
C	********************************
	CALL MGOCONNECT(FFX,YYT,IZCNT-1)
C	********************************
C
...
C	**************
	...
	WRITE(77,3010) 'QUANTITY:','TIME','E-FIELD','ANG TO SUN(X)',
     1 'FREQ(ANGLE)','FREQ(POWER)','POWER'
	WRITE(77,3099) 'UNITS:','s','mV/m','degrees','kHZ','kHZ',
     1 'dB V^2/HZ'
      WRITE(77,3014) 'BEGIN!'
	OC=1
 333  IF (OC.LE.2048.AND.OC.LE.(IZCNT-1).AND.OC.LE.NP)THEN
 	  WRITE(77,3002) OC,PP(OC),YY(OC),FFX(OC),YYT(OC),POWX(OC),POW(OC)
 	  OC=OC+1
 	  GOTO 333
 	ELSE IF (OC.LE.2048.AND.OC.LE.(IZCNT-1).AND.OC.GT.NP)THEN
 	  WRITE(77,3003) OC,PP(OC),YY(OC),FFX(OC),YYT(OC)
 	  OC=OC+1
 	  GOTO 333
 	ELSE IF (OC.GT.2048.AND.OC.LE.(IZCNT-1).AND.OC.LE.NP)THEN
 	  WRITE(77,3004) OC,FFX(OC),YYT(OC),POWX(OC),POW(OC)
 	  OC=OC+1
 	  GOTO 333
 	...
 	ELSE IF (OC.LE.2048.AND.OC.GT.(IZCNT-1).AND.OC.GT.NP)THEN
 	  ...
 	  OC=OC+1
 	  GOTO 333
 	ELSE IF (OC.GT.2048.AND.OC.LE.(IZCNT-1).AND.OC.GT.NP)THEN
 	  WRITE(77,3007) OC,FFX(OC),YYT(OC)
 	  OC=OC+1
 	  GOTO 333
...
C	DATFILE FORMATS
C	***************
 3002 FORMAT(T2,I4.4,T12,E12.6,T27,E12.6,T42,E12.6,T57,
     1 E12.6,T72,E12.6,T87,E12.6)
 3003 FORMAT(T2,I4.4,T12,E12.6,T27,E12.6,T42,E12.6,T57,E12.6) !12
 3004 FORMAT(T2,I4.4,T42,E12.6,T57,E12.6,T72,E12.6,T87,E12.6) !23
