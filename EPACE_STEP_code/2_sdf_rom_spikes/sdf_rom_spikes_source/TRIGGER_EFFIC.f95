...
c		7-Aug-95	modify include derf.f95 stmt to include
...
c
     ...
     1     fegain(2,100),thresh(2),fegain_now(2),dist(2),fract(2)
      ...
      character*120 trigger_effic_input
      character*120 stepmr_input
...
c	read input files -- first threshold data
	trigger_effic_input = adjustl(trim(calibration_path)) // adjustl(trim('trigger_effic.dat;1'))
      open(unit=33,name=trigger_effic_input,
...
c
	stepmr_input='/Users/mdesai/Desktop/IDL/wind/fortran_vax/2_sdf_rom_spikes/sdf_rom_spikes_source/STEPMRDEDX_NICKEL.KAL;8'
      open(unit=34,name=stepmr_input,
...
c
      open(unit=35,name=
     1    '/Users/mdesai/Desktop/IDL/wind/fortran_vax/2_sdf_rom_spikes/sdf_rom_spikes_source/STEP_GAIN_HISTORY.DAT;8',
     1    status='old',action='read')
