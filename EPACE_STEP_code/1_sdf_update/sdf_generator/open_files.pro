...
;		23-Jan-2009		increased kp file version max value to 9 /gm
;		11-Mar-2010		check that variable lun exists before freeing unit number /gm
;		19-Jan-2012		allow up to 20 version numbers of cdf (11/28/2011 had 10, exceeded old max value of 9) / gm
...
4:		type ='epa'
	endcase
		for j = 20, 1, -1 do begin
		kp_filename = '/Users/mdesai/Desktop/IDL/wind/step_kp_files/' + type + $
			'/wi_k0_' + type + string(year, month, day, j, format="('_',i4,2i2.2,'_v',i2.2,'.cdf')")
;		print, kp_filename
...
;		print, err
		...
		if( size(lun,/type) gt 0 ) then free_lun, lun             ; check that lun exists before executing this line  3/11/2010 /gm
