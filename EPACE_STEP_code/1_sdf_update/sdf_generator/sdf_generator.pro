...
;		23-Jan-2008		fix bug in non-batch prompt for days to process /gm
;
...
;		find the year and day range to process 
		if(batch_flag eq 0) then begin
			read, year_start, doy_start, year_stop, doy_stop, prompt='Enter year_start, doy_start, year_stop, doy_stop: '
...
;			if batch job, start date = last day processed + 1;  end date = last hex file date
		...
		if( run_stop_mjd lt run_start_mjd) then begin
			print, ' sdf_generator:  stop date is before start date; quitting early '
			exit, status = 2
		endif
...
;	now write out the last day processed if a batch job
		if(batch_flag eq 0) then begin
			print, 'last_day_processed file left unchanged!'
		endif else begin
